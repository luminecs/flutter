...
Future<void> main(List<String> args) async {
  ...
  final Snapshot snapshot =
      Snapshot.fromJson(jsonDecode(json) as Map<String, dynamic>);
  ...
  for (final String forbiddenType in options.forbiddenTypes) {
    ...
    if (!await validateType(forbiddenType, options.packageConfig)) {
      foundForbiddenTypes
          .add('Forbidden type "$forbiddenType" does not seem to exist.');
      continue;
    }
...
Future<bool> validateType(String forbiddenType, File packageConfigFile) async {
  ...
  final Uri packageUri =
      Uri.parse(forbiddenType.substring(0, forbiddenType.indexOf('::')));
  final String typeName =
      forbiddenType.substring(forbiddenType.indexOf('::') + 2);
  ...
  if (!packageFile.existsSync()) {
    print(
        'File $packageFile does not exist - forbidden type has moved or been removed.');
    return false;
  }
...
class Options {
  ...
  factory Options.fromArgs(List<String> args) {
    ...
    argParser.addOption(
      ...
      valueHelp: path.join(r'$FLUTTER_ROOT', 'examples', 'hello_world',
          '.dart_tool', 'package_config.json'),
      defaultsTo: path.join(fs.currentDirectory.path, 'examples', 'hello_world',
          '.dart_tool', 'package_config.json'),
    );
    ...
    return Options(
      ...
      forbiddenTypes:
          Set<String>.from(argResults['forbidden-type'] as List<String>),
    );
  }
