    ...
    try {
      ...
      try {
        ...
        await app.build(buildTarget,
            validateNativeBuildProject: !dartOnlyPlugin);
        if (runFlutterTest) {
          section('Test app');
...
''';
  ...
  Future<void> convertDefaultPluginToSharedDarwinPlugin() async {
    ...
    if (!pubspecContent.contains(originalIOSKey) ||
        !pubspecContent.contains(originalMacOSKey)) {
    ...
    pubspecContent = pubspecContent.replaceAll(
        originalIOSKey, '$originalIOSKey        sharedDarwinSource: true\n');
    pubspecContent = pubspecContent.replaceAll(originalMacOSKey,
    ...
    podspecContent =
        podspecContent.replaceFirst(RegExp(r'.*s\.platform.*'), '');
    podspecContent = podspecContent.replaceFirst("s.dependency 'Flutter'",
    ...
    final String pluginClass =
        '${name[0].toUpperCase()}${name.substring(1)}Plugin';
    print('pluginClass: $pluginClass');
    final File pluginRegister =
        File(path.join(darwinDir.path, 'Classes', '$pluginClass.swift'));
...
''';
  ...
  Future<void> runNativeTests(String buildTarget) async {
    ...
    switch (buildTarget) {
      ...
      case 'ios':
        await testWithNewIOSSimulator('TestNativeUnitTests',
      ...
      case 'linux':
        if (await exec(
              path.join(rootPath, 'build', 'linux', 'x64', 'release', 'plugins',
                  'plugintest', 'plugintest_test'),
              <String>[],
              canFail: true,
            ) !=
            0) {
      ...
      case 'windows':
        if (await exec(
              path.join(rootPath, 'build', 'windows', 'x64', 'plugins',
                  'plugintest', 'Release', 'plugintest_test.exe'),
              <String>[],
              canFail: true,
            ) !=
            0) {
...
''';
    ...
    podspecContent = podspecContent.replaceFirst(
        versionString, target == 'ios' ? iosContent : macosContent);
    podspec.writeAsStringSync(podspecContent, flush: true);
  }

  Future<void> build(String target,
      {bool validateNativeBuildProject = true}) async {
    await inDirectory(Directory(rootPath), () async {
      ...
      if (target == 'ios' || target == 'macos') {
        ...
        if (validateNativeBuildProject) {
          final File podsProject = File(path.join(
              rootPath, target, 'Pods', 'Pods.xcodeproj', 'project.pbxproj'));
          if (!podsProject.existsSync()) {
            throw TaskResult.failure(
                'Xcode Pods project file missing at ${podsProject.path}');
          }
          ...
          if (target == 'ios') {
            ...
            if (podsProjectContent
                .contains('IPHONEOS_DEPLOYMENT_TARGET = 10')) {
              throw TaskResult.failure(
            ...
            if (podsProjectContent
                .contains('MACOSX_DEPLOYMENT_TARGET = 10.8')) {
              throw TaskResult.failure(
            ...
            if (podsProjectContent
                .contains('MACOSX_DEPLOYMENT_TARGET = 10.9')) {
              throw TaskResult.failure(
