...
[Pub packages autoroller](https://github.com/flutter/flutter/blob/main/dev/conductor/core/bin/packages_autoroller.dart).''';
  ...
  late final Future<String> featureBranchName = (() async {
    final List<String> remoteBranches =
        await framework.listRemoteBranches(framework.mirrorRemote!.name);
  ...
  String _redactToken(String message) =>
      message.replaceAll(token, '[GitHub TOKEN]');
  ...
  Future<bool> updatePackages({
    ...
    if (exitCode != 0) {
      throw ConductorException(
          'Failed to update packages with exit code $exitCode');
    }
  ...
  Future<String> cli(
    ...
    final Future<void> stdoutFuture =
        process.stdout.transform(utf8.decoder).forEach(stdoutStrings.add);
    final Future<void> stderrFuture =
        process.stderr.transform(utf8.decoder).forEach(stderrStrings.add);
    if (stdin != null) {
