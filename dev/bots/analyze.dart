...
Future<void> main(List<String> arguments) async {
  final String dartSdk = path.join(
    ...
    _getDartSdkFromArguments(arguments) ??
...
Future<void> run(List<String> arguments) async {
  ...
  await runCommand(
    flutter,
    <String>['update-packages', '--verify-only'],
  ...
  final Directory outDir =
      Directory.systemTemp.createTempSync('flutter_mega_gallery.');
  try {
    await runCommand(
      dart,
      <String>[
    ...
    await _runFlutterAnalyze(outDir.path,
...
Future<void> verifyTargetPlatform(String workingDirectory) async {
  ...
  while (true) {
    if (index >= lines.length) {
      foundError(<String>[
        '${framework.path}: Can no longer find TargetPlatform enum.'
      ]);
      return;
    }
  ...
  while (true) {
    if (index >= lines.length) {
      foundError(<String>[
        '${framework.path}: Could not find end of TargetPlatform enum.'
      ]);
      return;
    }
  ...
  final File tool = File(
      '$workingDirectory/packages/flutter_tools/lib/src/resident_runner.dart');
  ...
  while (true) {
    if (index >= lines.length) {
      foundError(
          <String>['${tool.path}: Can no longer find nextPlatform logic.']);
      return;
    }
    if (lines[index]
        .trim()
        .startsWith('const List<String> platforms = <String>[')) {
      index += 1;
      break;
    }
    index += 1;
  ...
  while (true) {
    if (index >= lines.length) {
      foundError(
          <String>['${tool.path}: Could not find end of nextPlatform logic.']);
      return;
    }
  ...
  final Set<String> frameworkExtra =
      frameworkPlatforms.difference(toolPlatforms);
...
Future<void> verifyNoDoubleClamp(String workingDirectory) async {
  ...
  await for (final File file in testFiles) {
    ...
    for (final _Line clamp in visitor.clamps) {
      errors.add(
          '${file.path}:${clamp.line}: `clamp` method used instead of `clampDouble`.');
    }
  }
...
Future<void> verifyTokenTemplatesUpdateCorrectFiles(
    String workingDirectory) async {
  final List<String> errors = <String>[];

  String getMaterialDirPath(List<String> lines) {
    final String line =
        lines.firstWhere((String line) => line.contains('String materialLib'));
  ...
  String getFileName(String line) {
    ...
    final String leftClamp = line
        .substring(line.indexOf(materialLibString) + materialLibString.length);
    return leftClamp.substring(0, leftClamp.indexOf("'"));
  }

  final String genDefaultsBinDir =
      '$workingDirectory/dev/tools/gen_defaults/bin';
...
Future<void> verifyToolTestsEndInTestDart(String workingDirectory) async {
  ...
  final RegExp callsTestFunctionPattern =
      RegExp(r'(test\(.*\)|testUsingContext\(.*\)|testWithoutContext\(.*\))');

  await for (final File file
      in _allFiles(toolsTestPath, 'dart', minimumMatches: 300)) {
    ...
    final bool callsTestFunction =
        file.readAsStringSync().contains(callsTestFunctionPattern);
...
Future<void> verifyNoSyncAsyncStar(String workingDirectory,
    {int minimumMatches = 2000}) async {
  final RegExp syncPattern = RegExp(r'\s*?a?sync\*\s*?{');
  final RegExp ignorePattern =
      RegExp(r'^\s*?// The following uses a?sync\* because:? ');
  final RegExp commentPattern = RegExp(r'^\s*?//');
  final List<String> errors = <String>[];
  await for (final File file
      in _allFiles(workingDirectory, 'dart', minimumMatches: minimumMatches)) {
    ...
    for (int index = 0; index < lines.length; index += 1) {
      ...
      if (line.contains(syncPattern)) {
        ...
        while (lookBehindIndex >= 0 &&
            lines[lookBehindIndex].startsWith(commentPattern)) {
        ...
        if (!hasExplanation) {
          errors
              .add('${file.path}:$index: sync*/async* without an explanation.');
        }
      }
    }
  }
...
final RegExp _findGoldenDefinitionPattern =
    RegExp(r'matchesGoldenFile\(Object');
...
const String _ignoreGoldenTag =
    '// flutter_ignore: golden_tag (see analyze.dart)';
const String _ignoreGoldenTagForFile =
    '// flutter_ignore_for_file: golden_tag (see analyze.dart)';

Future<void> verifyGoldenTags(String workingDirectory,
    {int minimumMatches = 2000}) async {
  final List<String> errors = <String>[];
  await for (final File file
      in _allFiles(workingDirectory, 'dart', minimumMatches: minimumMatches)) {
    ...
    for (int lineNumber in linesWithDeprecations) {
      try {
        final RegExpMatch? startMatch =
            _deprecationStartPattern.firstMatch(lines[lineNumber]);
        if (startMatch == null) {
        ...
        do {
          final RegExpMatch? messageMatch =
              _deprecationMessagePattern.firstMatch(lines[lineNumber]);
          if (messageMatch == null) {
            ...
            if (lines[lineNumber].trimLeft().startsWith('"')) {
              ...
              if (lines[lineNumber].contains('This feature was deprecated')) {
                ...
                possibleReason =
                    ' There might be a missing space character at the end of the line.';
              }
            }
          ...
          versionMatch =
              _deprecationVersionPattern.firstMatch(lines[lineNumber]);
...
Future<void> verifyNoMissingLicense(String workingDirectory,
  ...
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'm',
      overrideMinimumMatches ?? 30, _generateLicense('// '));
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'cc',
      overrideMinimumMatches ?? 10, _generateLicense('// '));
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'cpp',
  ...
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'gradle',
      overrideMinimumMatches ?? 80, _generateLicense('// '));
  ...
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'bat',
      overrideMinimumMatches ?? 1, _generateLicense('REM '),
      header: r'@ECHO off\n');
  ...
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'html',
      overrideMinimumMatches ?? 1, '<!-- ${_generateLicense('')} -->',
      trailingBlank: false, header: r'<!DOCTYPE HTML>\n');
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'xml',
      overrideMinimumMatches ?? 1, '<!-- ${_generateLicense('')} -->',
      header: r'(<\?xml version="1.0" encoding="utf-8"\?>\n)?');
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'frag',
      overrideMinimumMatches ?? 1, _generateLicense('// '),
...
Future<void> _verifyNoMissingLicenseForExtension(
  ...
  final String licensePattern =
      RegExp.escape('$license\n${trailingBlank ? '\n' : ''}');
  final List<String> errors = <String>[];
  await for (final File file in _allFiles(workingDirectory, extension,
  ...
  // Fail if any errors
  if (errors.isNotEmpty) {
    final String fileDoes =
        errors.length == 1 ? 'file does' : '${errors.length} files do';
    foundError(<String>[
      ...
      if (header.isNotEmpty)
        'A header matching the regular expression "$header",',
...
Iterable<_Line> _getTestSkips(File file) {
  ...
  final _TestSkipLinesVisitor<CompilationUnit> visitor =
      _TestSkipLinesVisitor<CompilationUnit>(parseResult);
  visitor.visitCompilationUnit(parseResult.unit);
  ...
  await for (final File file in testFiles) {
    for (final _Line skip in _getTestSkips(file)) {
      ...
      if (skipComment == null ||
        ...
        errors.add(
            '${file.path}:${skip.line}: skip test without a justification comment.');
      }
    }
  }
...
final RegExp _testImportPattern =
    RegExp(r'''import (['"])([^'"]+_test\.dart)\1''');
...
Future<void> verifyNoTestImports(String workingDirectory) async {
  ...
  final List<File> dartFiles = await _allFiles(
          path.join(workingDirectory, 'packages'), 'dart',
          minimumMatches: 1500)
      .toList();
...
Future<void> verifyNoBadImportsInFlutter(String workingDirectory) async {
  ...
  final String libPath =
      path.join(workingDirectory, 'packages', 'flutter', 'lib');
  final String srcPath =
      path.join(workingDirectory, 'packages', 'flutter', 'lib', 'src');
  // Verify there's one libPath/*.dart for each srcPath/*/.
  final List<String> packages = Directory(libPath)
      .listSync()
  ...
  for (final String directory in directories) {
    dependencyMap[directory] = await _findFlutterDependencies(
        path.join(srcPath, directory), errors,
        checkForMeta: directory != 'foundation');
  }
  assert(dependencyMap['material']!.contains('widgets') &&
      dependencyMap['widgets']!.contains('rendering') &&
      dependencyMap['rendering']!.contains(
          'painting')); // to make sure we're convinced _findFlutterDependencies is finding some
  ...
  for (final String key in dependencyMap.keys) {
    for (final String dependency in dependencyMap[key]!) {
      ...
      // dependencies.
      final String validFilenames =
          dependencyMap.keys.map((String name) => '$name.dart').join(', ');
      errors.add('$key imported package:flutter/$dependency.dart '
  ...
  for (final String package in dependencyMap.keys) {
    ...
    if (loop != null) {
      errors
          .add('${yellow}Dependency loop:$reset ${loop.join(' depends on ')}');
    }
  }
  // Fail if any errors
  if (errors.isNotEmpty) {
...
Future<void> verifyIntegrationTestTimeouts(String workingDirectory) async {
  ...
  for (final File file in files) {
    ...
    final int timeoutNoneCount =
        'timeout: Timeout.none'.allMatches(contents).length;
...
Future<void> verifyInternationalizations(
    String workingDirectory, String dartExecutable) async {
  ...
  final String expectedMaterialResult =
      await File(materialLocalizationsFile).readAsString();
  final String expectedCupertinoResult =
      await File(cupertinoLocalizationsFile).readAsString();
...
Future<void> verifyNoCheckedMode(String workingDirectory) async {
  ...
  final List<File> files =
      await _allFiles(flutterPackages, 'dart', minimumMatches: 400)
...
Future<void> verifyNoRuntimeTypeInToString(String workingDirectory) async {
  final String flutterLib =
      path.join(workingDirectory, 'packages', 'flutter', 'lib');
  final Set<String> excludedFiles = <String>{
    path.join(flutterLib, 'src', 'foundation',
        'object.dart'), // Calls this from within an assert.
  };
  final List<File> files =
      await _allFiles(flutterLib, 'dart', minimumMatches: 400)
          .where((File file) => !excludedFiles.contains(file.path))
          .toList();
  final RegExp toStringRegExp =
      RegExp(r'^\s+String\s+to(.+?)?String(.+?)?\(\)\s+(\{|=>)');
  final List<String> problems = <String>[];
  for (final File file in files) {
    final List<String> lines = file.readAsLinesSync();
    for (int index = 0; index < lines.length; index++) {
      if (toStringRegExp.hasMatch(lines[index])) {
        ...
        bool checkForRuntimeType(String line) {
          if (line.contains(r'$runtimeType') ||
              line.contains('runtimeType.toString()')) {
            problems.add(
                '${file.path}:$sourceLine}: toString calls runtimeType.toString');
            return true;
          }
          return false;
        }
        ...
        if (lines[index].contains('=>')) {
          while (!lines[index].contains(';')) {
            ...
            assert(index < lines.length,
                'Source file $file has unterminated toString method.');
            if (checkForRuntimeType(lines[index])) {
              break;
            }
          }
        } else {
          int openBraceCount = '{'.allMatches(lines[index]).length -
...
Future<void> verifyNoTrailingSpaces(String workingDirectory,
    {int minimumMatches = 4000}) async {
  final List<File> files = await _allFiles(workingDirectory, null,
          minimumMatches: minimumMatches)
...
final RegExp _flowControlStatementWithoutSpace =
    RegExp(r'(^|[ \t])(if|switch|for|do|while|catch)\(', multiLine: true);

Future<void> verifySpacesAfterFlowControlStatements(String workingDirectory,
    {int minimumMatches = 4000}) async {
  ...
  for (final File file in files) {
    ...
    for (int index = 0; index < lines.length; index += 1) {
      if (lines[index].contains(_flowControlStatementWithoutSpace)) {
        problems.add(
            '${file.path}:${index + 1}: no space after flow control statement');
      }
    }
  }
  if (problems.isNotEmpty) {
...
Future<void> verifyIssueLinks(String workingDirectory) async {
  const String issueLinkPrefix =
      'https://github.com/flutter/flutter/issues/new';
  const Set<String> stops = <String>{'\n', ' ', "'", '"', r'\', ')', '>'};
  assert(!stops.contains(
      '.')); // instead of "visit https://foo." say "visit: https://", it copy-pastes better
  ...
  for (final File file in files) {
    ...
    while ((start = contents.indexOf(issueLinkPrefix, start)) >= 0) {
      ...
      if (url == issueLinkPrefix) {
        if (file.path !=
            path.join(workingDirectory, 'dev', 'bots', 'analyze.dart')) {
          problems
              .add('${file.path} contains a direct link to $issueLinkPrefix.');
        ...
        final List<String>? templates =
            parsedUrl.queryParametersAll['template'];
        if (templates == null) {
          problems.add(
              '${file.path} contains $url, which has no "template" argument specified.');
          suggestions.add(kGiveTemplates);
          suggestions.add(kTemplates);
...
final Set<Hash256> _legacyBinaries = <Hash256>{
  ...
  const Hash256(0x7770183009E91411, 0x2DE7D8EF1D235A6A, 0x30C5834424858E0D,
      0x2F8253F6B8D31926),
  ...
  // (also used by many examples)
  const Hash256(0x5925DAB509451F9E, 0xCBB12CE8A625F9D4, 0xC104718EE20CAFF8,
      0xB1B51032D1CD8946),
  ...
  // (also used by many examples)
  const Hash256(0xC4D9A284C12301D0, 0xF50E248EC53ED51A, 0x19A10147B774B233,
      0x08399250B0D44C55),
  ...
  // (also used by many examples)
  const Hash256(0xBF97F9D3233F33E1, 0x389B09F7B8ADD537, 0x41300CB834D6C7A5,
      0xCA32CBED363A4FB2),
  ...
  // (also used by many examples)
  const Hash256(0x285442F69A06B45D, 0x9D79DF80321815B5, 0x46473548A37B7881,
      0x9B68959C7B8ED237),
  ...
  // (also used by many examples)
  const Hash256(0x2AB64AF8AC727EA9, 0x9C6AB9EAFF847F46, 0xFBF2A9A0A78A0ABC,
      0xBF3180F3851645B4),
  ...
  // (also used by many examples)
  const Hash256(0x9DCA09F4E5ED5684, 0xD3C4DFF41F4E8B7C, 0xB864B438172D72BE,
      0x069315FA362930F9),
  ...
  // (also used by many examples)
  const Hash256(0xD5AD04DE321EF37C, 0xACC5A7B960AFCCE7, 0x1BDCB96FA020C482,
      0x49C1545DD1A0F497),
  ...
  // (also used by many examples)
  const Hash256(0x809ABFE75C440770, 0xC13C4E2E46D09603, 0xC22053E9D4E0E227,
      0x5DCB9C1DCFBB2C75),
  ...
  // (also used by many examples)
  const Hash256(0x3DB08CB79E7B01B9, 0xE81F956E3A0AE101, 0x48D0FAFDE3EA7AA7,
      0x0048DF905AA52CFD),
  ...
  // (also used by many examples)
  const Hash256(0x23C13D463F5DCA5C, 0x1F14A14934003601, 0xC29F1218FD461016,
      0xD8A22CEF579A665F),
  ...
  // (also used by many examples)
  const Hash256(0x6DB7726530D71D3F, 0x52CB59793EB69131, 0x3BAA04796E129E1E,
      0x043C0A58A1BFFD2F),
  ...
  // (also used by many examples)
  const Hash256(0xCEE565F5E6211656, 0x9B64980B209FD5CA, 0x4B3D3739011F5343,
      0x250B33A1A2C6EB65),
  ...
  const Hash256(0x3DCE99077602F704, 0x21C1C6B2A240BC9B, 0x83D64D86681D45F2,
      0x154143310C980BE3),
  ...
  const Hash256(0x6E645DC9ED913AAD, 0xB50ED29EEB16830D, 0xB32CA12F39121DB9,
      0xB7BC1449DDDBF8B8),
  ...
  const Hash256(0xDEFAC77E08EC71EC, 0xA04CCA3C95D1FC33, 0xB9F26E1CB15CB051,
      0x47DEFC79CDD7C158),

  // examples/flutter_view/ios/Runner/ic_add.png
  // examples/platform_view/ios/Runner/ic_add.png
  const Hash256(0x3CCE7450334675E2, 0xE3AABCA20B028993, 0x127BE82FE0EB3DFF,
      0x8B027B3BAF052F2F),

  // examples/image_list/images/coast.jpg
  const Hash256(0xDA957FD30C51B8D2, 0x7D74C2C918692DC4, 0xD3C5C99BB00F0D6B,
      0x5EBB30395A6EDE82),

  // examples/image_list/ios/Runner/Assets.xcassets/AppIcon.appiconset/Icon-App-1024x1024@1x.png
  const Hash256(0xB5792CA06F48A431, 0xD4379ABA2160BD5D, 0xE92339FC64C6A0D3,
      0x417AA359634CD905),

  // TEST ASSETS

  // dev/benchmarks/macrobenchmarks/assets/999x1000.png
  const Hash256(0x553E9C36DFF3E610, 0x6A608BDE822A0019, 0xDE4F1769B6FBDB97,
      0xBC3C20E26B839F59),

  // dev/bots/test/analyze-test-input/root/packages/foo/serviceaccount.enc
  const Hash256(0xA8100AE6AA1940D0, 0xB663BB31CD466142, 0xEBBDBD5187131B92,
      0xD93818987832EB89),

  // dev/automated_tests/icon/test.png
  const Hash256(0xE214B4A0FEEEC6FA, 0x8E7AA8CC9BFBEC40, 0xBCDAC2F2DEBC950F,
      0x75AF8EBF02BCE459),
  ...
  const Hash256(0x2D4F8D7A3DFEF9D3, 0xA0C66938E169AB58, 0x8C6BBBBD1973E34E,
      0x03C428416D010182),
  ...
  const Hash256(0xCD46C01BAFA3B243, 0xA6AA1645EEDDE481, 0x143AC8ABAB1A0996,
      0x22CAA9D41F74649A),

  // dev/integration_tests/flutter_driver_screenshot_test/assets/red_square.png
  const Hash256(0x40054377E1E084F4, 0x4F4410CE8F44C210, 0xABA945DFC55ED0EF,
      0x23BDF9469E32F8D3),

  // dev/integration_tests/flutter_driver_screenshot_test/test_driver/goldens/red_square_image/iPhone7,2.png
  const Hash256(0x7F9D27C7BC418284, 0x01214E21CA886B2F, 0x40D9DA2B31AE7754,
      0x71D68375F9C8A824),

  // examples/flutter_view/assets/flutter-mark-square-64.png
  // examples/platform_view/assets/flutter-mark-square-64.png
  const Hash256(0xF416B0D8AC552EC8, 0x819D1F492D1AB5E6, 0xD4F20CF45DB47C22,
      0x7BB431FEFB5B67B2),

  // packages/flutter_tools/test/data/intellij/plugins/Dart/lib/Dart.jar
  const Hash256(0x576E489D788A13DB, 0xBF40E4A39A3DAB37, 0x15CCF0002032E79C,
      0xD260C69B29E06646),

  // packages/flutter_tools/test/data/intellij/plugins/flutter-intellij.jar
  const Hash256(0x4C67221E25626CB2, 0x3F94E1F49D34E4CF, 0x3A9787A514924FC5,
      0x9EF1E143E5BC5690),

  // MISCELLANEOUS

  // dev/bots/serviceaccount.enc
  const Hash256(0x1F19ADB4D80AFE8C, 0xE61899BA776B1A8D, 0xCA398C75F5F7050D,
      0xFB0E72D7FBBBA69B),

  // dev/docs/favicon.ico
  const Hash256(0x67368CA1733E933A, 0xCA3BC56EF0695012, 0xE862C371AD4412F0,
      0x3EC396039C609965),

  // dev/snippets/assets/code_sample.png
  const Hash256(0xAB2211A47BDA001D, 0x173A52FD9C75EBC7, 0xE158942FFA8243AD,
      0x2A148871990D4297),

  // dev/snippets/assets/code_snippet.png
  const Hash256(0xDEC70574DA46DFBB, 0xFA657A771F3E1FBD, 0xB265CFC6B2AA5FE3,
      0x93BA4F325D1520BA),

  // packages/flutter_tools/static/Ahem.ttf
  const Hash256(0x63D2ABD0041C3E3B, 0x4B52AD8D382353B5, 0x3C51C6785E76CE56,
      0xED9DACAD2D2E31C4),
};

Future<void> verifyNoBinaries(String workingDirectory,
    {Set<Hash256>? legacyBinaries}) async {
  // Please do not add anything to the _legacyBinaries set above.
  // We have a policy of not checking in binaries into this repository.
  // If you are adding/changing template images, use the flutter_template_images
  // package and a .img.tmpl placeholder instead.
  // If you have other binaries to add, please consult Hixie for advice.
  assert(_legacyBinaries
              .expand<int>(
                  (Hash256 hash) => <int>[hash.a, hash.b, hash.c, hash.d])
...
Future<List<File>> _gitFiles(String workingDirectory,
  ...
  return filenames
      .map<File>(
          (String filename) => File(path.join(workingDirectory, filename)))
      .toList();
}
...
Future<void> _checkConsumerDependencies() async {
  ...
  for (final String package in kCorePackages) {
    ...
    final Map<String, Map<String, Object?>> dependencyTree =
        <String, Map<String, Object?>>{
      for (final Map<String, Object?> package
          in (rawJson['packages']! as List<Object?>)
...
final RegExp _nullInitializedField =
    RegExp(r'kDebugMode \? [\w<> ,{}()]+ : null;');

Future<void> verifyNullInitializedDebugExpensiveFields(String workingDirectory,
    {int minimumMatches = 400}) async {
  final String flutterLib =
      path.join(workingDirectory, 'packages', 'flutter', 'lib');
  final List<File> files =
      await _allFiles(flutterLib, 'dart', minimumMatches: minimumMatches)
          .toList();
  final List<String> errors = <String>[];
  for (final File file in files) {
    final List<String> lines = file.readAsLinesSync();
...
final RegExp tabooPattern =
    RegExp(r'^ *///.*\b(simply|note:|note that)\b', caseSensitive: false);

Future<void> verifyTabooDocumentation(String workingDirectory,
    {int minimumMatches = 100}) async {
  final List<String> errors = <String>[];
  await for (final File file
      in _allFiles(workingDirectory, 'dart', minimumMatches: minimumMatches)) {
    final List<String> lines = file.readAsLinesSync();
...
final String _kIntegrationTestsRelativePath =
    path.join('dev', 'integration_tests');
final String _kTemplateRelativePath = path.join('packages', 'flutter_tools',
    'templates', 'app_shared', 'windows.tmpl', 'runner');
...
final String _kLicensePath = path.join(
    'dev', 'conductor', 'core', 'lib', 'src', 'proto', 'license_header.txt');
...
Future<void> verifyIntegrationTestTemplateFiles(String flutterRoot) async {
  ...
  final String integrationTestsPath =
      path.join(flutterRoot, _kIntegrationTestsRelativePath);
  ...
  for (final Directory testPath in subDirs) {
    ...
    final Iterable<File> files =
        Directory(templatePath).listSync().toList().whereType<File>();
    for (final File templateFile in files) {
      ...
      if (fileName.endsWith(_kTmplExt)) {
        appFilePath = appFilePath.substring(
            0,
            appFilePath.length -
                _kTmplExt.length); // Remove '.tmpl' from app file path
        templateFileContents = templateFileContents.replaceAll(
            _kProjectNameKey, projectName); // Substitute template project name
      }
...
final RegExp _importPattern =
    RegExp(r'''^\s*import (['"])package:flutter/([^.]+)\.dart\1''');
final RegExp _importMetaPattern =
    RegExp(r'''^\s*import (['"])package:meta/meta\.dart\1''');

Future<Set<String>> _findFlutterDependencies(
    String srcPath, List<String> errors,
    {bool checkForMeta = false}) async {
