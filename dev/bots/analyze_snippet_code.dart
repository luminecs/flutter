...
class _AnalysisError extends _ErrorBase {
  ...
  @override
  int get hashCode =>
      Object.hash(file, line, column, message, errorCode, source);
}
...
class _SnippetChecker {
  ...
  static final RegExp _dartDocSnippetBeginRegex =
      RegExp(r'{@tool ([^ }]+)(?:| ([^}]*))}');
  ...
  static final RegExp _uncheckedCodeBlockStartRegex =
      RegExp(r'^ *// *```dart$');
  ...
  static final RegExp _namedArgumentRegExp =
      RegExp(r'^(?:  )?([a-zA-Z0-9_]+): ');

  static final RegExp _topLevelDeclarationRegExp =
      RegExp(r'^(abstract|class|mixin|enum|typedef|final|extension) ');
  ...
  Future<bool> checkSnippets() async {
    ...
    if (_dartUiLocation != null && !_dartUiLocation.existsSync()) {
      stderr.writeln(
          'Unable to analyze engine dart snippets at ${_dartUiLocation.path}.');
    }
  ...
  static Directory _createTempDirectory(String? tempArg) {
    if (tempArg != null) {
      final Directory tempDirectory = Directory(path.join(
          Directory.systemTemp.absolute.path, path.basename(tempArg)));
  ...
  Future<List<Object>> _extractSnippets(
    ...
    for (final File file in files) {
      try {
        ...
        bool inDartSection =
            false; // Whether or not we're in a '```dart' segment.
        ...
        for (final String line in fileLines) {
          ...
          if (inExamplesCanAssumePreamble) {
            if (line.isEmpty) {
              ...
              final _Line newLine =
                  _Line(line: lineNumber, indent: 3, code: line.substring(3));
            ...
            final RegExpMatch? snippetMatch =
                _dartDocSnippetBeginRegex.firstMatch(trimmedLine);
  ...
  _SnippetFile _processBlock(
    ...
    final List<_Line> headers =
        switch ((importPreviousExample, customImports.length)) {
      (true, _) => <_Line>[],
    ...
    if (hasStatefulWidgetComment) {
      return _SnippetFile.fromStrings(
        ...
        block.any(
            (String line) => line.startsWith(_topLevelDeclarationRegExp))) {
      ...
      if (firstCodeLine.startsWith(_namedArgumentRegExp)) {
        ...
        for (int index = 0; index < block.length; index += 1) {
          final Match? prefix =
              _namedArgumentRegExp.matchAsPrefix(block[index]);
          if (prefix != null) {
      ...
      for (int index = block.length - 1; index >= 0; index -= 1) {
        if (!block[index].startsWith(_nonCodeRegExp)) {
          final Match? lastLine =
              _trailingCommaRegExp.matchAsPrefix(block[index]);
          if (lastLine != null) {
  ...
  void _createConfigurationFiles() {
    final File targetPubSpec =
        File(path.join(_tempDirectory.path, 'pubspec.yaml'));
    if (!targetPubSpec.existsSync()) {
      // Copying pubspec.yaml from examples/api into temp directory.
      final File sourcePubSpec =
          File(path.join(_flutterRoot, 'examples', 'api', 'pubspec.yaml'));
    ...
    final File targetAnalysisOptions =
        File(path.join(_tempDirectory.path, 'analysis_options.yaml'));
  ...
  File _writeSnippetFile(_SnippetFile snippetFile) {
    final String snippetFileId = _createNameFromSource(
        'snippet', snippetFile.filename, snippetFile.indexLine);
    final File outputFile =
        File(path.join(_tempDirectory.path, '$snippetFileId.dart'))
          ..createSync(recursive: true);
    ...
    for (final String error in analyzerOutput) {
      ...
      if (match == null) {
        errors.add(_SnippetCheckerException(
            'Could not parse analyzer output: $error'));
        continue;
      }
      ...
      while (true) {
        ...
        if ((lineNumber - delta > 0) &&
          ...
          actualColumn =
              delta == 0 ? columnNumber : actualSource.code.length + 1;
          actualMessage =
              delta == 0 ? message : '$message -- in later generated code';
          break;
        }
...
class _SnippetFile {
  const _SnippetFile(
      this.code, this.generatorComment, this.filename, this.indexLine);
  ...
  factory _SnippetFile.fromStrings(
    ...
    final List<_Line> codeLines = <_Line>[
      ...
      for (int i = 0; i < code.length; i += 1)
        _Line(
            code: code[i], line: firstLine.line + i, indent: firstLine.indent),
      if (postfix != null) _Line.generated(code: postfix),
    ];
    return _SnippetFile.fromLines(
        codeLines, headers, preamble, generatorComment, filename);
  }
...
Future<void> _runInteractive({
  ...
  if (!file.existsSync()) {
    stderr.writeln(
        'Specified file ${file.absolute.path} does not exist or is not a file.');
    exit(1);
  }
  ...
  final _SnippetChecker checker =
      _SnippetChecker(flutterPackages, tempDirectory: tempDirectory)
        .._createConfigurationFiles();
  ...
  Future<void> rerun() async {
    ...
    try {
      ...
      final Set<Object> errors = <Object>{};
      errors.addAll(
          await checker._extractSnippets(<File>[file], snippetMap: snippets));
