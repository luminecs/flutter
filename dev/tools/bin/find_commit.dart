...
class Commit {
  ...
  static Commit parse(String line) {
    ...
    return Commit(line.substring(0, space),
        DateTime.parse(line.substring(space + 1, line.length).trimRight()));
  }
...
String findCommit({
  ...
  if (primaryBranch == primaryTrunk) {
    ...
    anchor = Commit.parse(git(primaryRepoDirectory, <String>[
      'log',
      Commit.formatArgument,
      '--max-count=1',
      primaryBranch,
      '--'
    ]));
  } else {
    final List<Commit> branchCommits = Commit.parseList(git(
        primaryRepoDirectory,
        <String>['log', Commit.formatArgument, primaryBranch, '--']));
    final List<Commit> trunkCommits = Commit.parseList(git(primaryRepoDirectory,
        <String>['log', Commit.formatArgument, primaryTrunk, '--']));
    if (branchCommits.isEmpty ||
        trunkCommits.isEmpty ||
        branchCommits.first.hash != trunkCommits.first.hash) {
      throw StateError(
    ...
    if (branchCommits.last.hash == trunkCommits.last.hash) {
      ...
      while (branchCommits.length > index &&
          ...
          branchCommits[index].hash == trunkCommits[index].hash) {
        index += 1;
      }
...
String git(String workingDirectory, List<String> arguments) {
  ...
  if (result.exitCode != 0 || '${result.stderr}'.isNotEmpty) {
    throw ProcessException(
        'git', arguments, '${result.stdout}${result.stderr}', result.exitCode);
  }
...
void main(List<String> arguments) {
  if (arguments.isEmpty ||
    ...
    print(findCommit(
      ...
      primaryBranch:
          git(primaryRepo, <String>['rev-parse', '--abbrev-ref', 'HEAD'])
              .trim(),
      primaryTrunk: 'master',
