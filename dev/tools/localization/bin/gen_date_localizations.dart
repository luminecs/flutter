...
Future<void> main(List<String> rawArgs) async {
  ...
  final File packageConfigFile = File(path.join('packages',
      'flutter_localizations', '.dart_tool', 'package_config.json'));
  ...
  if (pathToIntl == null) {
    exitWithError('Could not find "intl" package. $_kCommandName must be run '
        'after a successful "flutter update-packages".');
  }
...
/// supported by flutter_localizations.''');
  ...
  if (writeToFile) {
    final File dateLocalizationsFile = File(path.join(
        'packages',
...
Map<String, File> _listIntlData(Directory directory) {
  ...
  locales.sort();
  return Map<String, File>.fromIterable(locales,
      value: (dynamic locale) => localeFiles[locale]!);
}
