...
List<T> _getGrandchildList<T>(
    Map<String, dynamic> map, String key1, String key2) {
...
class LogicalKeyData {
  factory LogicalKeyData(
    ...
    _readWindowsKeyCodes(
        data, windowsKeyCodeHeader, parseMapOfListOfString(windowsNameMap));
    _readGtkKeyCodes(
        data, gtkKeyCodeHeader, parseMapOfListOfString(gtkNameMap));
    _readAndroidKeyCodes(
        data, androidKeyCodeHeader, parseMapOfListOfString(androidNameMap));
    _readMacOsKeyCodes(
        data, physicalKeyData, parseMapOfListOfString(macosLogicalToPhysical));
    _readIosKeyCodes(
        data, physicalKeyData, parseMapOfListOfString(iosLogicalToPhysical));
    _readFuchsiaKeyCodes(data, physicalKeyData);
    _readGlfwKeyCodes(
        data, glfwHeaderFile, parseMapOfListOfString(glfwNameMap));
    // Sort entries by value
    final List<MapEntry<String, LogicalKeyEntry>> sortedEntries =
        data.entries.toList()
          ..sort(
            (MapEntry<String, LogicalKeyEntry> a,
                    MapEntry<String, LogicalKeyEntry> b) =>
  ...
  factory LogicalKeyData.fromJson(Map<String, dynamic> contentMap) {
    ...
    data.addEntries(contentMap.values.map((dynamic value) {
      final LogicalKeyEntry entry =
          LogicalKeyEntry.fromJsonMapEntry(value as Map<String, dynamic>);
  ...
  LogicalKeyEntry entryByName(String name) {
    assert(
        _data.containsKey(name), 'Unable to find logical entry by name $name.');
    return _data[name]!;
  }
  ...
  static Map<String, LogicalKeyEntry> _readKeyEntries(String input) {
    ...
    for (final RegExpMatch match in domKeyRegExp.allMatches(input)) {
      ...
      final int entryValue =
          toPlane(value, _sourceToPlane(source, isPrintable));
  ...
  static void _readMacOsKeyCodes(
    ...
    final Map<String, String> physicalToLogical = reverseMapOfListOfString(
        logicalToPhysical, (String logicalKeyName, String physicalKeyName) {
    ...
    physicalToLogical.forEach((String physicalKeyName, String logicalKeyName) {
      final PhysicalKeyEntry physicalEntry =
          physicalKeyData.entryByName(physicalKeyName);
  ...
  static void _readGtkKeyCodes(Map<String, LogicalKeyEntry> data,
      String headerFile, Map<String, List<String>> nameToGtkName) {
    final RegExp definedCodes = RegExp(
      r'#define '
      r'GDK_KEY_(?<name>[a-zA-Z0-9_]+)\s*'
      r'0x(?<value>[0-9a-f]+),?',
    );
    final Map<String, String> gtkNameToFlutterName = reverseMapOfListOfString(
        nameToGtkName, (String flutterName, String gtkName) {
  ...
  static void _readWindowsKeyCodes(Map<String, LogicalKeyEntry> data,
      String headerFile, Map<String, List<String>> nameMap) {
    // The mapping from the Flutter name (e.g. "enter") to the Windows name (e.g.
    // "RETURN").
    final Map<String, String> nameToFlutterName = reverseMapOfListOfString(
        nameMap, (String flutterName, String windowsName) {
    ...
    for (final RegExpMatch match in definedCodes.allMatches(headerFile)) {
      ...
      if (entry == null) {
        print(
            'Invalid logical entry by name $name (from Windows $windowsName)');
        continue;
      }
      addNameValue(
        entry.windowsNames,
  ...
  static void _readAndroidKeyCodes(Map<String, LogicalKeyEntry> data,
      String headerFile, Map<String, List<String>> nameMap) {
    final Map<String, String> nameToFlutterName = reverseMapOfListOfString(
        nameMap, (String flutterName, String androidName) {
    ...
    headerFile = headerFile.replaceAllMapped(
        enumBlock, (Match match) => match.group(1)!);
  ...
  static void _readFuchsiaKeyCodes(
      Map<String, LogicalKeyEntry> data, PhysicalKeyData physicalData) {
    for (final LogicalKeyEntry entry in data.values) {
      final int? value = (() {
        ...
        if (keyLabel != null && !entry.constantName.startsWith('numpad')) {
          ...
          final PhysicalKeyEntry? physicalEntry =
              physicalData.tryEntryByName(entry.name);
  ...
  static void _readGlfwKeyCodes(Map<String, LogicalKeyEntry> data,
      String headerFile, Map<String, List<String>> nameMap) {
    final Map<String, String> nameToFlutterName = reverseMapOfListOfString(
        nameMap, (String flutterName, String glfwName) {
  ...
  static final Map<String, _ModifierPair> _chromeModifiers = () {
    final String rawJson = File(path.join(
      dataRoot,
    ...
    return (json.decode(rawJson) as Map<String, dynamic>)
      ...
      return MapEntry<String, _ModifierPair>(
          key, _ModifierPair(pair[0] as String, pair[1] as String));
    });
  }();

  static final Map<String, String> printable = (() {
    final String printableKeys = File(path.join(
      dataRoot,
      'printable.json',
    )).readAsStringSync();
  ...
  static final Map<String, List<String>> synonyms = (() {
    final String synonymKeys = File(path.join(
      dataRoot,
      'synonyms.json',
    )).readAsStringSync();
    ...
    return dynamicSynonym.map((String name, dynamic values) {
      ...
      final List<String> names =
          (values as List<dynamic>).whereType<String>().toList();
...
class LogicalKeyEntry {
  ...
  static String computeCommentName(String name) {
    ...
    return replaced
        ...
        // 'AB1' => 'AB 1', 'F1' => 'F1'
        .replaceAllMapped(RegExp(r'([A-Z]{2,})([0-9])'),
            (Match match) => '${match.group(1)} ${match.group(2)}')
        // 'Foo1' => 'Foo 1'
        .replaceAllMapped(RegExp(r'([a-z])([0-9])'),
            (Match match) => '${match.group(1)} ${match.group(2)}')
        .trim();
  }
