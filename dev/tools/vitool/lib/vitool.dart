...
class Animation {
  ...
  static void _validateFramesData(List<FrameData> frames) {
    ...
    for (int i = 0; i < frames.length; i += 1) {
      ...
      if (size != frame.size) {
        throw Exception('All animation frames must have the same size,\n'
...
class PathAnimation {
  ...
  factory PathAnimation.fromFrameData(List<FrameData> frames, int pathIdx) {
    if (frames.isEmpty) {
      return const PathAnimation(<PathCommandAnimation>[],
          opacities: <double>[]);
    }

    final List<PathCommandAnimation> commands = <PathCommandAnimation>[];
    for (int commandIdx = 0;
        commandIdx < frames[0].paths[pathIdx].commands.length;
        commandIdx += 1) {
      final int numPointsInCommand =
          frames[0].paths[pathIdx].commands[commandIdx].points.length;
      final List<List<Point<double>>> points = List<List<Point<double>>>.filled(
          numPointsInCommand, <Point<double>>[]);
      final String commandType =
          frames[0].paths[pathIdx].commands[commandIdx].type;
      for (int i = 0; i < frames.length; i += 1) {
        final FrameData frame = frames[i];
        final String currentCommandType =
            frame.paths[pathIdx].commands[commandIdx].type;
...
FrameData interpretSvg(String svgFilePath) {
  ...
  final double width =
      parsePixels(_extractAttr(svgElement, 'width')).toDouble();
  final double height =
      parsePixels(_extractAttr(svgElement, 'height')).toDouble();
    ...
    if (element.name.local == 'g') {
      ...
      if (_hasAttr(element, 'transform')) {
        transformMatrix = transformMatrix
            .multiplied(_parseSvgTransform(_extractAttr(element, 'transform')));
      }
...
class SvgPath {
  ...
  SvgPath _applyTransform(_Transform transform) {
    final List<SvgPathCommand> transformedCommands = commands
        ...
        .toList();
    return SvgPath(id, transformedCommands,
  ...
  SvgPathCommand _applyTransform(_Transform transform) {
    final List<Point<double>> transformedPoints = _vector3ArrayToPoints(
        transform.transformMatrix
            .applyToVector3Array(_pointsToVector3Array(points)));
...
class SvgPathCommandBuilder {
  ...
  SvgPathCommand build(String type, List<Point<double>> points) {
    ...
    if (_isRelativeCommand(type)) {
      absPoints = points
          .map<Point<double>>((Point<double> p) => p + lastPoint)
          .toList();
    }
...
int parsePixels(String pixels) {
  if (!_pixelsExp.hasMatch(pixels)) {
    throw ArgumentError("illegal pixels expression: '$pixels'"
...
String _extractAttr(XmlElement element, String name) {
  try {
    return element.attributes
        .singleWhere((XmlAttribute x) => x.name.local == name)
        .value;
  } catch (e) {
    throw ArgumentError(
...
bool _hasAttr(XmlElement element, String name) {
  return element.attributes
      .where((XmlAttribute a) => a.name.local == name)
      .isNotEmpty;
}

XmlElement _extractSvgElement(XmlDocument document) {
  return document.children.singleWhere((XmlNode node) =>
      node.nodeType == XmlNodeType.ELEMENT &&
      _asElement(node).name.local == 'svg') as XmlElement;
}
