...
void main() {
  ...
  testUsingContext('Downgrade exits on unknown channel', () async {
    final FakeFlutterVersion fakeFlutterVersion =
        FakeFlutterVersion(branch: 'WestSideStory'); // an unknown branch
    ...
    final DowngradeCommand command = DowngradeCommand(
      persistentToolState: PersistentToolState.test(
          directory: fileSystem.currentDirectory, logger: bufferLogger),
    ...
    expect(
        ...
        throwsToolExit(
            message: 'Flutter is not currently on a known channel.'));
  });
  ...
  testUsingContext('Downgrade does not prompt when there is no terminal',
      () async {
    final FakeFlutterVersion fakeFlutterVersion = FakeFlutterVersion();
...
class FakeTerminal extends Fake implements Terminal {
  ...
  Future<String> promptForCharInput(List<String> acceptedCharacters,
      ...
      bool displayAcceptedCharacters = true}) async {
