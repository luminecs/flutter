...
void main() {
  ...
  group('run', () {
    ...
    testUsingContext('fails when target not found', () async {
      ...
      expect(
        () => createTestCommandRunner(command)
            .run(<String>['run', '-t', 'abc123', '--no-pub']),
    ...
    testUsingContext('does not support --no-sound-null-safety by default',
        () async {
      fileSystem.file('lib/main.dart').createSync(recursive: true);
      fileSystem.file('pubspec.yaml').createSync();
      fileSystem.file('.packages').createSync();

      final TestRunCommandThatOnlyValidates command =
    ...
    testUsingContext(
      ...
      final FakeDevice device =
          FakeDevice(isLocalEmulator: true, platformType: PlatformType.android);
    ...
    group('run app', () {
      ...
      setUp(() {
        ...
        fs.currentDirectory.childFile('.packages').writeAsStringSync(
      ...
      testUsingContext(
        ...
        await expectLater(
          ...
          throwsA(isA<ToolExit>()
              .having((ToolExit error) => error.message, 'message', isNull)),
        );
      ...
      testUsingContext(
        ...
        expect(testLogger.statusText,
            contains('The following devices were found:'));
        expect(testLogger.statusText,
      ...
      testUsingContext(
        ...
        final FakeDevice device = FakeDevice(
            isLocalEmulator: true, platformType: PlatformType.android);
      ...
      testUsingContext('fails when v1 FlutterApplication is detected',
          () async {
      ...
      testUsingContext('forwards --uninstall-only to DebuggingOptions',
          () async {
        final RunCommand command = RunCommand();
        final FakeDevice mockDevice = FakeDevice(
        ...
        final DebuggingOptions options =
            await command.createDebuggingOptions(false);
      ...
      group('--machine', () {
        ...
        testUsingContext('can pass --device-user', () async {
          final DaemonCapturingRunCommand command = DaemonCapturingRunCommand();
          final FakeDevice device =
              FakeDevice(platformType: PlatformType.android);
          testDeviceManager.devices = <Device>[device];
    ...
    group('Fatal Logs', () {
      ...
      testUsingContext(
          'fails if --fatal-warnings specified and warnings emitted', () async {
        testLogger.printWarning('Warning: Mild annoyance Will Robinson!');
        await expectLater(
            createTestCommandRunner(command).run(<String>[
              'run',
    ...
    group('usageValues', () {
      testUsingContext('with only non-iOS usb device', () async {
        ...
        final TestRunCommandForUsageValues command =
            TestRunCommandForUsageValues(devices: devices);
  ...
  group('dart-defines and web-renderer options', () {
    ...
    test('auto web-renderer with no dart-defines', () {
      dartDefines =
          FlutterCommand.updateDartDefines(dartDefines, WebRendererMode.auto);
      expect(dartDefines, <String>['FLUTTER_WEB_AUTO_DETECT=true']);
    });

    test('canvaskit web-renderer with no dart-defines', () {
      dartDefines = FlutterCommand.updateDartDefines(
          dartDefines, WebRendererMode.canvaskit);
    ...
    test('html web-renderer with no dart-defines', () {
      dartDefines =
          FlutterCommand.updateDartDefines(dartDefines, WebRendererMode.html);
      expect(dartDefines, <String>[
        'FLUTTER_WEB_AUTO_DETECT=false',
  ...
  group('terminal', () {
    ...
    testUsingContext(
        'Flutter run sets terminal singleCharMode to false on exit', () async {
      final FakeResidentRunner residentRunner = FakeResidentRunner();
      final TestRunCommandWithFakeResidentRunner command =
  ...
  testUsingContext(
      ...
      () async {
    final FakeResidentRunner residentRunner = FakeResidentRunner();
    residentRunner.rpcError =
        RPCError('flutter._listViews', RPCErrorCodes.kServiceDisappeared, '');
    final TestRunCommandWithFakeResidentRunner command =
        TestRunCommandWithFakeResidentRunner();
    command.fakeResidentRunner = residentRunner;
  ...
  testUsingContext('Flutter run does not catch other RPC errors', () async {
    final FakeResidentRunner residentRunner = FakeResidentRunner();
    residentRunner.rpcError =
        RPCError('flutter._listViews', RPCErrorCodes.kInvalidParams, '');
    final TestRunCommandWithFakeResidentRunner command =
        TestRunCommandWithFakeResidentRunner();
    command.fakeResidentRunner = residentRunner;
...
class FakeDevice extends Fake implements Device {
  ...
  Never _throwToolExit(int code) =>
      throwToolExit('FakeDevice tool exit', exitCode: code);
  ...
  Future<String> get sdkNameAndVersion =>
      Future<String>.value(_sdkNameAndVersion);
...
class FakeAnsiTerminal extends Fake implements AnsiTerminal {
  ...
  set singleCharMode(bool value) {
    if (!hasStdin) {
      throw const StdinException('Error setting terminal line mode',
