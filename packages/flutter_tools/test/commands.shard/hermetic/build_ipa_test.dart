...
''');
  ...
  FakeCommand setUpFakeXcodeBuildHandler(
      {bool verbose = false, int exitCode = 0, void Function()? onRun}) {
    return FakeCommand(
      command: <String>[
        'xcrun',
        'xcodebuild',
  ...
  testUsingContext('ipa build reports when IPA fails', () async {
    ...
    expect(
        testLogger.statusText, contains('build/ios/archive/Runner.xcarchive'));
...
[
  ...
  testUsingContext('Display xcresult issues on console if parsed.', () async {
    ...
    expect(
        testLogger.errorText, contains("Use of undeclared identifier 'asdas'"));
    expect(
        testLogger.errorText,
        contains(
