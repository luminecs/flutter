...
void main() {
  ...
  testUsingContext(
    ...
    final PackagesGetCommand command =
        PackagesGetCommand('get', '', PubContext.pubGet);
  ...
  testUsingContext('pub get on target directory', () async {
    ...
    final FlutterProject rootProject =
        FlutterProject.fromDirectory(targetDirectory);
    expect(rootProject.packageConfigFile.existsSync(), true);
    expect(await rootProject.packageConfigFile.readAsString(),
  ...
  testUsingContext('pub get throws error on missing directory', () async {
    ...
    try {
      ...
      expect(e.toString(),
          contains('Expected to find project root in missing_dir'));
    }
  }, overrides: <Type, Generator>{
    Pub: () => pub,
    ProcessManager: () => FakeProcessManager.any(),
