...
void main() {
  ...
  group('packages get/upgrade', () {
    ...
    Future<PackagesCommand> runCommandIn(String projectPath, String verb,
    ...
    void expectContains(String projectPath, String relPath, String substring) {
      ...
      expect(
        globals.fs
            .file(globals.fs.path.join(projectPath, relPath))
            .readAsStringSync(),
        contains(substring),
    ...
    void removeGeneratedFiles(String projectPath) {
      ...
      for (final String path in allFiles) {
        final File file =
            globals.fs.file(globals.fs.path.join(projectPath, path));
    ...
    testUsingContext('get fetches packages and has output from pub', () async {
      ...
      expect(
        ...
        allOf(
          contains(matches(
              RegExp(r'Resolving dependencies in .+flutter_project\.\.\.'))),
          contains(matches(RegExp(r'\+ flutter 0\.0\.0 from sdk flutter'))),
          contains(matches(
              RegExp(r'Changed \d+ dependencies in .+flutter_project!'))),
        ),
      );
    ...
    testUsingContext(
      ...
      pubspecFileContent =
          pubspecFileContent.replaceFirst(RegExp(r'\nflutter\:'), '''
flutter:
  generate: true
''');
      projectDir
    ...
    testUsingContext('set no plugins as usage value', () async {
      ...
      final PackagesGetCommand getCommand =
          command.subcommands['get']! as PackagesGetCommand;
    ...
    testUsingContext('set the number of plugins as usage value', () async {
      ...
      final String exampleProjectPath =
          globals.fs.path.join(projectPath, 'example');

      final PackagesCommand command =
    ...
    testUsingContext('indicate that the project is not a module in usage value',
        () async {
      final String projectPath =
      ...
      expect(
          (await getCommand.usageValues).commandPackagesProjectModule, false);
    }, overrides: <Type, Generator>{
      Stdio: () => mockStdio,
      Pub: () => Pub.test(
            fileSystem: globals.fs,
    ...
    testUsingContext('indicate that the project is a module in usage value',
        () async {
      final String projectPath = await createProject(tempDir,
          arguments: <String>['--no-pub', '--template=module']);
      removeGeneratedFiles(projectPath);
    ...
    testUsingContext('indicate that Android project reports v1 in usage value',
        () async {
      final String projectPath =
          await createProject(tempDir, arguments: <String>['--no-pub']);
      removeGeneratedFiles(projectPath);
      ...
      final String updatedAndroidManifestString = androidManifest
          .readAsStringSync()
          .replaceAll('android:value="2"', 'android:value="1"');
      ...
      expect(
          (await getCommand.usageValues).commandPackagesAndroidEmbeddingVersion,
          'v1');
    }, overrides: <Type, Generator>{
      Stdio: () => mockStdio,
      Pub: () => Pub.test(
    ...
    testUsingContext('indicate that Android project reports v2 in usage value',
        () async {
      final String projectPath =
          await createProject(tempDir, arguments: <String>['--no-pub']);
      removeGeneratedFiles(projectPath);
