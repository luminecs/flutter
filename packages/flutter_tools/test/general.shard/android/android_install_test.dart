...
const FakeCommand kAdbStartServerCommand =
    FakeCommand(command: <String>['adb', 'start-server']);
...
const FakeCommand kStoreShaCommand = FakeCommand(command: <String>[
  'adb',
  '-s',
  '1234',
  'shell',
...
void main() {
  ...
  testWithoutContext('Can install app on API level 16 or greater', () async {
    ...
    expect(
        await androidDevice.installApp(androidApk, userIdentifier: '10'), true);
    expect(processManager, hasNoRemainingExpectations);
  });

  testWithoutContext('Defaults to API level 16 if adb returns a null response',
      () async {
    final FakeProcessManager processManager =
        FakeProcessManager.list(<FakeCommand>[
      kAdbVersionCommand,
  ...
  testWithoutContext('displays error if user not found', () async {
    ...
    expect(await androidDevice.installApp(androidApk, userIdentifier: 'jane'),
        false);
    expect(
        logger.errorText,
        contains(
  ...
  testWithoutContext('Will skip install if the correct version is up to date',
      () async {
    final FakeProcessManager processManager =
        FakeProcessManager.list(<FakeCommand>[
      kAdbVersionCommand,
  ...
  testWithoutContext(
    ...
    expect(await androidDevice.installApp(androidApk, userIdentifier: '10'),
        false);
    expect(processManager, hasNoRemainingExpectations);
  });
}
