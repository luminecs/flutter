...
void main() {
  group('remove link and embed migration', () {
    ...
    testWithoutContext('skipped if files are missing', () {
      final RemoveMacOSFrameworkLinkAndEmbeddingMigration
          macosProjectMigration = RemoveMacOSFrameworkLinkAndEmbeddingMigration(
        macOSProject,
        testLogger,
        testUsage,
      );
...
''');
  ...
  group('update deployment target version', () {
    ...
    testWithoutContext('skipped if files are missing', () {
      final MacOSDeploymentTargetMigration macOSProjectMigration =
...
''');
  ...
  group('update NSPrincipalClass from FlutterApplication to NSApplication', () {
    ...
    testWithMocks('skipped if files are missing', () async {
      final FlutterApplicationMigration macOSProjectMigration =
    ...
    testWithMocks('skipped if no NSPrincipalClass key exists to upgrade',
        () async {
      final FlutterApplicationMigration macOSProjectMigration =
          FlutterApplicationMigration(
        project,
      ...
      infoPlistFile.writeAsStringSync(
          'contents'); // Just so it exists: parser is a fake.
      macOSProjectMigration.migrate();
      expect(
          fakePlistParser.getValueFromFile<String>(
              infoPlistFile.path, PlistParser.kNSPrincipalClassKey),
          isNull);
      expect(testLogger.statusText, isEmpty);
    });

    testWithMocks('skipped if already de-upgraded (or never migrated)',
        () async {
      fakePlistParser.setProperty(
          PlistParser.kNSPrincipalClassKey, 'NSApplication');
      final FlutterApplicationMigration macOSProjectMigration =
          FlutterApplicationMigration(
        project,
        testLogger,
    ...
    testWithMocks('Info.plist migrated to use NSApplication', () async {
      fakePlistParser.setProperty(
          PlistParser.kNSPrincipalClassKey, 'FlutterApplication');
      final FlutterApplicationMigration macOSProjectMigration =
          FlutterApplicationMigration(
        project,
        testLogger,
    ...
    testWithMocks('Skip if NSPrincipalClass is not NSApplication', () async {
      ...
      fakePlistParser.setProperty(
          PlistParser.kNSPrincipalClassKey, differentApp);
      final FlutterApplicationMigration macOSProjectMigration =
          FlutterApplicationMigration(
        project,
        testLogger,
