...
void main() {
  testUsingContext('Copies assets to expected directory after building',
      () async {
    ...
    expect(
        globals.fs
            .file(globals.fs.path.join('example', 'kernel_blob.bin'))
            .existsSync(),
        true);
    expect(
        globals.fs
            .file(globals.fs.path.join('example', 'LICENSE'))
            .existsSync(),
        true);
    expect(
        globals.fs.file(globals.fs.path.join('example.d')).existsSync(), false);
  ...
  testUsingContext('Passes correct defines to build system', () async {
    ...
    expect(env!.defines[kFrontendServerStarterPath],
    ...
    expect(env!.defines[kDartDefines],
        encodeDartDefines(<String>['test8', 'test9']));
  ...
  testWithoutContext(
      '--enable-experiment is removed from getDefaultCachedKernelPath hash',
      () {
