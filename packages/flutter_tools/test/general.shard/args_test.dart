...
void main() {
  ...
  test(
      ...
      () => Testbed().run(() {
            final FlutterCommandRunner runner =
                FlutterCommandRunner(verboseHelp: true);
            executable
                .generateCommands(
                  verboseHelp: true,
                  verbose: true,
  ...
  testUsingContext('Global arg results are available in FlutterCommands',
      () async {
    ...
    expect(
        command.boolArg(FlutterGlobalOptions.kContinuousIntegrationFlag,
            global: true),
        true);
  });

  testUsingContext(
      'Global arg results are available in FlutterCommands sub commands',
      () async {
    final DummyFlutterCommand command = DummyFlutterCommand(
      commandFunction: () async {
        return const FlutterCommandResult(ExitStatus.success);
    ...
    expect(
        subcommand.boolArg(FlutterGlobalOptions.kContinuousIntegrationFlag,
            global: true),
        true);
  });
  ...
  testUsingContext('bool? safe argResults', () async {
    ...
    expect(
        () => command.boolArg('key'), throwsA(const TypeMatcher<TypeError>()));
    ...
    expect(command.boolArg('key'), true);
    expect(() => command.boolArg('non-existent'),
  ...
  testUsingContext('String? safe argResults', () async {
    ...
    expect(command.stringArg('key'), 'value');
    expect(() => command.stringArg('non-existent'),
        throwsA(const TypeMatcher<ArgumentError>()));
  });
  ...
  testUsingContext('List<String> safe argResults', () async {
    ...
    expect(() => command.stringsArg('non-existent'),
        throwsA(const TypeMatcher<ArgumentError>()));
...
void verifyCommandRunner(CommandRunner<Object?> runner) {
  expect(runner.argParser, isNotNull,
      reason: '${runner.runtimeType} has no argParser');
  expect(runner.argParser.allowsAnything, isFalse,
      reason: '${runner.runtimeType} allows anything');
  expect(runner.argParser.allowTrailingOptions, isFalse,
...
final RegExp _allowedArgumentNamePatternForPrecache =
    RegExp(r'^([-a-z0-9_]+)$');
...
final RegExp _allowedTrailingPatterns =
    RegExp(r'([^ ]([^.^!^:][.!:])\)?|: https?://[^ ]+[^.]|^)$');
...
final RegExp _nonSecureFlutterDartUrl =
    RegExp(r'http://([a-z0-9-]+\.)*(flutter|dart)\.dev', caseSensitive: false);
...
void verifyOptions(String? command, Iterable<Option> options) {
  ...
  for (final Option option in options) {
    ...
    if (command == 'precache') {
      expect(option.name, matches(_allowedArgumentNamePatternForPrecache),
          reason:
              '$_header$target--${option.name}" is not a valid name for a command line argument. (Is it all lowercase?)');
    } else {
      expect(option.name, matches(_allowedArgumentNamePattern),
          reason:
    ...
    expect(option.name, isNot(matches(_bannedArgumentNamePattern)),
        reason:
    ...
    if (option.name != FlutterOptions.kNullSafety &&
        option.name != FlutterOptions.kNullAssertions) {
      expect(option.hide, isFalse,
          reason:
              '${_header}Help for $target--${option.name}" is always hidden. $_needHelp');
    }
    expect(option.help, isNotNull,
        reason:
            '${_header}Help for $target--${option.name}" has null help. $_needHelp');
    expect(option.help, isNotEmpty,
        reason:
            '${_header}Help for $target--${option.name}" has empty help. $_needHelp');
    expect(option.help, isNot(matches(_bannedLeadingPatterns)),
        reason:
    ...
    expect(option.help, isNot(startsWith('(Deprecated')),
        reason:
    ...
    expect(option.help, isNot(matches(_questionablePatterns)),
        reason:
    ...
    if (option.defaultsTo != null) {
      expect(option.help, isNot(contains('Default')),
          reason:
    ...
    for (final String line in option.help!.split('\n')) {
      if (!line.startsWith('    ')) {
        ...
        expect(line, matches(_allowedTrailingPatterns),
            reason:
    ...
    expect(option.help, isNot(endsWith(':')),
        reason:
            '${_header}Help for $target--${option.name}" ends with a colon, which seems unlikely to be correct.');
    expect(option.help, isNot(contains(_bannedUri)),
        reason:
            '${_header}Help for $target--${option.name}" uses the term "URI" rather than "URL".');
    expect(option.help, isNot(contains(_nonSecureFlutterDartUrl)),
        reason:
