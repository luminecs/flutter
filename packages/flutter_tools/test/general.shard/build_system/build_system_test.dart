...
void main() {
  ...
  testWithoutContext(
      'Does not throw exception if asked to build with missing inputs',
      () async {
  ...
  testWithoutContext('Saves a stamp file with inputs and outputs', () async {
    ...
    final File stampFile =
        fileSystem.file('${environment.buildDir.path}/foo.stamp');

    expect(stampFile, exists);

    final Map<String, Object?>? stampContents =
        castStringKeyedMap(json.decode(stampFile.readAsStringSync()));
  ...
  testWithoutContext('does not re-invoke build if input timestamp changes',
      () async {
    final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
    await buildSystem.build(fooTarget, environment);
  ...
  testWithoutContext('does not re-invoke build if output timestamp changes',
      () async {
    final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
    await buildSystem.build(fooTarget, environment);
  ...
  testWithoutContext('Re-invoke build if output is modified', () async {
    ...
    environment.buildDir
        .childFile('out')
        .writeAsStringSync('Something different');
  ...
  testWithoutContext(
      'Automatically cleans old outputs when build graph changes', () async {
    final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
  ...
  testWithoutContext('Does not crash when filesystem and cache are out of sync',
      () async {
    final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
  ...
  testWithoutContext('handles a throwing build action without crashing',
      () async {
    final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
  ...
  testWithoutContext(
      'Target with depfile dependency will not run twice without '
    ...
    final BuildResult result =
        await buildSystem.buildIncremental(target, environment, null);
  ...
  testWithoutContext('Additional inputs do not change the build configuration',
      () async {
    final Environment environmentA = Environment.test(
        fileSystem.currentDirectory,
        artifacts: Artifacts.test(),
  ...
  testWithoutContext(
    ...
    final TestTarget releaseTarget =
        TestTarget((Environment environment) async {
    ...
    expect(
        testEnvironmentProfile.outputDir.childFile('.last_build_id'), exists);
    expect(
        testEnvironmentProfile.outputDir
  ...
  testWithoutContext('A target using canSkip can create a conditional output',
      () async {
    final BuildSystem buildSystem = setUpBuildSystem(fileSystem);
    ...
    expect(foo, exists);
    expect(secondResult.inputFiles.map((File file) => file.path),
        <String>[bar.path]);
    expect(secondResult.outputFiles.map((File file) => file.path),
        <String>[foo.path]);

    // bar is destroyed, foo is also deleted.
    bar.deleteSync();
    final BuildResult thirdResult =
        await buildSystem.build(target, environment);
  ...
  testWithoutContext('Build completes all dependencies before failing',
      () async {
    ...
    final BuildSystem buildSystem = setUpBuildSystem(
        ...
        FakePlatform(
          numberOfProcessors:
              10, // Ensure the tool will process tasks concurrently.
        ));
...
class TestTarget extends Target {
  TestTarget(
      [Future<void> Function(Environment environment)? build, this._canSkip])
