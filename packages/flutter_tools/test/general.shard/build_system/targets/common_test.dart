...
''';
  ...
  testWithoutContext(
      'KernelSnapshot correctly forwards FrontendServerStarterPath', () async {
    fileSystem.file('.dart_tool/package_config.json')
      ..createSync(recursive: true)
      ..writeAsStringSync('{"configVersion": 2, "packages":[]}');
    androidEnvironment.buildDir.childFile('native_assets.yaml')
    ...
    await const KernelSnapshot().build(androidEnvironment
      ..defines[kFrontendServerStarterPath] =
          'path/to/frontend_server_starter.dart');
  ...
  testWithoutContext(
    ...
    await const KernelSnapshot().build(androidEnvironment
      ..defines[kTargetPlatform] =
          getNameForTargetPlatform(TargetPlatform.darwin)
      ..defines[kBuildMode] = BuildMode.debug.cliName
      ..defines[kTrackWidgetCreation] = 'false');
  ...
  testUsingContext(
      ...
      () async {
    expect(
        const AotAssemblyProfile().build(androidEnvironment), throwsException);
  }, overrides: <Type, Generator>{
    Platform: () => macPlatform,
    FileSystem: () => fileSystem,
    ProcessManager: () => processManager,
