...
void main() {
  ...
  test(
      ...
      () => testbed.run(() async {
            ...
            final String generated =
                environment.buildDir.childFile('main.dart').readAsStringSync();
            ...
            // Import.
            expect(generated,
                contains("import 'package:foo/main.dart' as entrypoint;"));
  ...
  test(
      ...
      () => testbed.run(() async {
            ...
            await const WebReleaseBundle(WebRendererMode.auto, isWasm: false)
                .build(environment);
...
''');
            webResources.childFile('foo.txt').writeAsStringSync('A');
            environment.buildDir.childFile('main.dart.js').createSync();
            ...
            expect(
                environment.outputDir.childFile('foo.txt').readAsStringSync(),
                'A');
            expect(environment.outputDir.childFile('main.dart.js').existsSync(),
                true);
            expect(
                environment.outputDir
                    .childDirectory('assets')
                    .childFile('AssetManifest.json')
                    .existsSync(),
                true);
      ...
      () => testbed.run(() async {
            final File mainFile =
                globals.fs.file(globals.fs.path.join('foo', 'lib', 'main.dart'))
                  ...
                  ..writeAsStringSync('void main() {}');
            globals.fs
                .file(globals.fs.path.join('pubspec.yaml'))
  ...
  test(
      ...
      () => testbed.run(() async {
            environment.defines[kBuildMode] = 'profile';
            environment.defines[kExtraFrontEndOptions] =
  ...
  test(
      ...
      () => testbed.run(() async {
            environment.defines[kBuildMode] = 'release';
            environment.defines[JsCompilerConfig.kNativeNullAssertions] =
                'true';
            processManager.addCommand(FakeCommand(command: <String>[
              ..._kDart2jsLinuxArgs,
              '-Ddart.vm.product=true',
  ...
  test(
      ...
      () => testbed.run(() async {
            ...
            final Depfile depfile = environment.depFileService
                .parse(environment.buildDir.childFile('dart2js.d'));

            expect(
                depfile.inputs.single.path, globals.fs.path.absolute('a.dart'));
  ...
  test(
      ...
      () => testbed.run(() async {
            environment.defines[kBuildMode] = 'release';
            environment.defines[kDartDefines] =
                encodeDartDefines(<String>['FOO=bar', 'BAZ=qux']);
            processManager.addCommand(FakeCommand(command: <String>[
              ..._kDart2jsLinuxArgs,
              '-Ddart.vm.product=true',
              '-DFOO=bar',
  ...
  test(
      ...
      () => testbed.run(() async {
            environment.defines[kBuildMode] = 'profile';
            environment.defines[
                JsCompilerConfig.kDart2jsNoFrequencyBasedMinification] = 'true';
            processManager.addCommand(FakeCommand(command: <String>[
              ..._kDart2jsLinuxArgs,
              '-Ddart.vm.profile=true',
              '--no-source-maps',
  ...
  test(
      'Dart2WasmTarget invokes dart2wasm with dart defines',
      () => testbed.run(() async {
            environment.defines[kBuildMode] = 'profile';
            environment.defines[WasmCompilerConfig.kRunWasmOpt] =
                WasmOptLevel.defaultValue.name;
            environment.defines[kDartDefines] =
                encodeDartDefines(<String>['FOO=bar', 'BAZ=qux']);

            final File depFile = environment.buildDir.childFile('dart2wasm.d');

            final File outputJsFile =
                environment.buildDir.childFile('main.dart.unopt.mjs');
            processManager.addCommand(FakeCommand(
                command: <String>[
                  ...
                  environment.buildDir.childFile('main.dart').absolute.path,
                  environment.buildDir
                      .childFile('main.dart.unopt.wasm')
                      .absolute
                      .path,
                ],
                onRun: () => outputJsFile
  ...
  test(
      'Dart2WasmTarget invokes dart2wasm and wasm-opt with debug info in wasmopt debug mode',
      () => testbed.run(() async {
            environment.defines[kBuildMode] = 'release';
            environment.defines[WasmCompilerConfig.kRunWasmOpt] =
                WasmOptLevel.debug.name;
  ...
  test(
      'Dart2WasmTarget invokes dart2wasm (but not wasm-opt) with wasm-opt none option',
      () => testbed.run(() async {
            environment.defines[kBuildMode] = 'debug';
            environment.defines[WasmCompilerConfig.kRunWasmOpt] =
                WasmOptLevel.none.name;
  ...
  test(
      ...
      () => testbed.run(() async {
            environment.outputDir.childDirectory('a').childFile('a.txt')
              ..createSync(recursive: true)
              ..writeAsStringSync('A');
            await WebServiceWorker(globals.fs, WebRendererMode.auto,
                    isWasm: false)
                .build(environment);

            expect(environment.outputDir.childFile('flutter_service_worker.js'),
                exists);
            // Contains file hash.
            expect(
                environment.outputDir
  ...
  test(
      ...
      () => testbed.run(() {
            ...
            expect(flutterJsContents,
                contains('if (!("serviceWorker" in navigator))'));
            ...
            expect(
                flutterJsContents, contains('document.querySelector("base")'));
  ...
  test(
      ...
      () => testbed.run(() async {
            ...
            await WebBuiltInAssets(globals.fs, WebRendererMode.auto,
                    isWasm: false)
                .build(environment);
  ...
  test(
      ...
      () => testbed.run(() async {
            globals.fs
                .file('bin/cache/flutter_web_sdk/canvaskit/canvaskit.wasm')
                .createSync(recursive: true);
  ...
  test(
      'wasm copies over canvaskit again if the web sdk changes',
      () => testbed.run(() async {
            final File canvasKitInput = globals.fs
                .file('bin/cache/flutter_web_sdk/canvaskit/canvaskit.wasm')
              ..createSync(recursive: true);
