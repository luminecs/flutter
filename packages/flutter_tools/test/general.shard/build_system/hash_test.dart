...
void main() {
  ...
  testWithoutContext('md5 control test zero length string', () {
    ...
    expect(hex.encode(hash.finalize().buffer.asUint8List()),
  ...
  testWithoutContext('Can hash bytes less than 64 length', () {
    ...
    expect(
        hashA.finalize(), <int>[1810219370, 268668871, 3900423769, 1277973076]);

    final Md5Hash hashB = Md5Hash();
    hashB.addChunk(bytes);

    expect(
        hashB.finalize(), <int>[1810219370, 268668871, 3900423769, 1277973076]);
  });
  ...
  testWithoutContext('Can hash bytes exactly 64 length', () {
    ...
    expect(
        hashA.finalize(), <int>[260592333, 2557619848, 2729912077, 812879060]);

    final Md5Hash hashB = Md5Hash();
    hashB.addChunk(bytes);

    expect(
        hashB.finalize(), <int>[260592333, 2557619848, 2729912077, 812879060]);
  });
  ...
  testWithoutContext('Can hash bytes more than 64 length', () {
    ...
    expect(
        hashA.finalize(), <int>[387658779, 2003142991, 243395797, 1487291259]);

    final Md5Hash hashB = Md5Hash();
    hashB.addChunk(bytes);

    expect(
        hashB.finalize(), <int>[387658779, 2003142991, 243395797, 1487291259]);
  });
}
