...
Map<String, dynamic> copyJsonObjectWith(
        Map<String, dynamic> object, Map<String, dynamic> overrides) =>
...
void main() {
  testWithoutContext(
    ...
    final CustomDevicesConfig customDevicesConfig = CustomDevicesConfig.test(
        fileSystem: fileSystem, directory: directory, logger: logger);
  ...
  testWithoutContext(
    ...
    writeCustomDevicesConfigFile(directory, json: <String, dynamic>{
      'test': 'testvalue',
    });
    ...
    expect(() => customDevicesConfig.devices,
        throwsA(const CustomDeviceRevivalException(msg)));
    expect(logger.errorText, contains(msg));
  });

  testWithoutContext('CustomDeviceRevivalException serialization', () {
    expect(const CustomDeviceRevivalException('testmessage').toString(),
        equals('testmessage'));
    expect(
        const CustomDeviceRevivalException.fromDescriptions(
                'testfielddescription', 'testexpectedvaluedescription')
            .toString(),
        equals(
            'Expected testfielddescription to be testexpectedvaluedescription.'));
  });

  testWithoutContext(
      'CustomDevicesConfig can load test config and logs no errors', () {
    final BufferLogger logger = BufferLogger.test();
    final MemoryFileSystem fileSystem = MemoryFileSystem.test();
    final Directory directory = fileSystem.directory('custom_devices_config');
  ...
  testWithoutContext(
      'CustomDevicesConfig logs error when label is not a string', () {
    final BufferLogger logger = BufferLogger.test();
    final MemoryFileSystem fileSystem = MemoryFileSystem.test();
    final Directory directory = fileSystem.directory('custom_devices_config');
