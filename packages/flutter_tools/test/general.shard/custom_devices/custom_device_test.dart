...
void _writeCustomDevicesConfigFile(
    Directory dir, List<CustomDeviceConfig> configs) {
  dir.createSync();
...
void main() {
  testWithoutContext(
      'replacing string interpolation occurrences in custom device commands',
      () async {
    expect(
        interpolateCommand(
          <String>[
  ...
  final CustomDeviceConfig disabledTestConfig =
      testConfig.copyWith(enabled: false);
  ...
  testUsingContext('CustomDevice defaults', () async {
  ...
  testWithoutContext(
    ...
    _writeCustomDevicesConfigFile(
        dir, <CustomDeviceConfig>[disabledTestConfig]);
  ...
  testWithoutContext(
      'CustomDevice.isSupportedForProject is true with editable host app',
      () async {
    ...
    final FlutterProject flutterProject =
        _setUpFlutterProject(fileSystem.currentDirectory);
  ...
  testUsingContext('CustomDevice.install invokes uninstall and install command',
      () async {
    ...
    final CustomDevice device = CustomDevice(
        ...
        processManager: FakeProcessManager.list(<FakeCommand>[
          ...
          FakeCommand(
              command: testConfig.installCommand,
              onRun: () => bothCommandsWereExecuted = true),
        ]));
  ...
  testWithoutContext(
    ...
    final FakeProcessManager processManager =
      ...
      FakeCommand(
        ...
        stdout:
            'The Dart VM service is listening on http://127.0.0.1:12345/abcd/\n',
      ),
      FakeCommand(
        command: testConfig.forwardPortCommand!,
    ...
    expect(
        launchResult.vmServiceUri, Uri.parse('http://127.0.0.1:12345/abcd/'));
  ...
  testWithoutContext(
    ...
    final FakeProcessManager processManager =
      ...
      FakeCommand(
          ...
          completer: runDebugCompleter,
          stdout:
              'The Dart VM service is listening on http://192.168.178.123:12345/abcd/\n'),
    ]);
    ...
    expect(launchResult.started, true);
    expect(launchResult.vmServiceUri,
        Uri.parse('http://192.168.178.123:12345/abcd/'));
    expect(runDebugCompleter.isCompleted, false);
  ...
  testUsingContext('custom device end-to-end test', () async {
    final Completer<void> runDebugCompleter = Completer<void>();
    final Completer<void> forwardPortCompleter = Completer<void>();
    ...
    final FakeProcessManager processManager =
        FakeProcessManager.list(<FakeCommand>[
      FakeCommand(
          command: testConfig.pingCommand, stdout: testConfigPingSuccessOutput),
    ...
    _writeCustomDevicesConfigFile(
        configFileDir, <CustomDeviceConfig>[testConfig]);
  ...
  testWithoutContext(
    ...
    final Iterable<List<int>> lines =
        Iterable<List<int>>.generate(5, (int _) => utf8.encode('test'));
    ...
    logReader.logLines.listen((_) {}, onDone: () {
      expect(subscriptions,
          everyElement((MyFakeStreamSubscription<String> s) => s.canceled));
    ...
    logReader.subscriptions.replaceRange(
      ...
      logReader.subscriptions.map((StreamSubscription<String> e) =>
          MyFakeStreamSubscription<String>(e)),
    );
...
class MyFakeStreamSubscription<T> extends Fake
