    ...
    group('run', () {
      testUsingContext('checks that Flutter installation is up-to-date',
        ...
        final FlutterCommandRunner runner =
            createTestCommandRunner(DummyFlutterCommand())
                as FlutterCommandRunner;
        final FakeFlutterVersion version =
            globals.flutterVersion as FakeFlutterVersion;
      ...
      testUsingContext(
        ...
        await runner.run(
            <String>['dummy', '--version', '--machine', '--version-check']);
      ...
      testUsingContext(
        ...
        final FlutterCommand command =
            DummyFlutterCommand(name: 'bash-completion');
        final FlutterCommandRunner runner =
      ...
      group('getRepoPackages', () {
        ...
        setUp(() {
          ...
          fileSystem
              .directory(
                  fileSystem.path.join(_kFlutterRoot, 'dev', 'tools', 'aatool'))
              .createSync(recursive: true);

          fileSystem
              .file(fileSystem.path
                  .join(_kFlutterRoot, 'dev', 'tools', 'pubspec.yaml'))
              .createSync();
          fileSystem
              .file(fileSystem.path.join(
                  _kFlutterRoot, 'dev', 'tools', 'aatool', 'pubspec.yaml'))
              .createSync();
        });
          ...
          expect(packagePaths, <String>[
            fileSystem
                .directory(fileSystem.path
                    .join(_kFlutterRoot, 'dev', 'tools', 'aatool'))
                .path,
            fileSystem
                .directory(fileSystem.path.join(_kFlutterRoot, 'dev', 'tools'))
                .path,
          ]);
        }, overrides: <Type, Generator>{
          FileSystem: () => fileSystem,
