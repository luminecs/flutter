...
final String automatedTestsDirectory =
    fileSystem.path.join('..', '..', 'dev', 'automated_tests');
final String missingDependencyDirectory =
    fileSystem.path.join('..', '..', 'dev', 'missing_dependency_tests');
final String flutterTestDirectory =
    fileSystem.path.join(automatedTestsDirectory, 'flutter_test');
final String integrationTestDirectory =
    fileSystem.path.join(automatedTestsDirectory, 'integration_test');
...
void main() {
  ...
  testWithoutContext('flutter test should not have extraneous error messages',
      () async {
    return _testFile(
  ...
  testWithoutContext(
      'integration test should not have extraneous error messages', () async {
    return _testFile(
        'trivial_widget', automatedTestsDirectory, integrationTestDirectory,
        exitCode: isZero, extraArguments: integrationTestExtraArgs);
  });

  testWithoutContext('flutter test set the working directory correctly',
      () async {
    return _testFile(
        'working_directory', automatedTestsDirectory, flutterTestDirectory,
        exitCode: isZero);
  });
  ...
  testWithoutContext(
      ...
      () async {
    return _testFile(
        'exception_handling', automatedTestsDirectory, integrationTestDirectory,
  ...
  testWithoutContext(
      ...
      () async {
    return _testFile('print_user_created_ancestor', automatedTestsDirectory,
        flutterTestDirectory,
  ...
  testWithoutContext(
      ...
      () async {
    return _testFile('print_user_created_ancestor_no_flag',
  ...
  testWithoutContext(
      ...
      () async {
    return _testFile('print_correct_local_widget', automatedTestsDirectory,
        flutterTestDirectory,
        extraArguments: const <String>['--track-widget-creation']);
  });

  testWithoutContext(
      'flutter test should can load assets within its own package', () async {
    return _testFile(
        'package_assets', automatedTestsDirectory, flutterTestDirectory,
        exitCode: isZero);
  });

  testWithoutContext('flutter test should support dart defines', () async {
    return _testFile(
        'dart_defines', automatedTestsDirectory, flutterTestDirectory,
        exitCode: isZero,
  ...
  testWithoutContext('flutter test should run a test with a given tag',
      () async {
    final ProcessResult result = await _runFlutterTest(
      'filtering_tag',
      automatedTestsDirectory,
  ...
  testWithoutContext('flutter test should not run a test with excluded tag',
      () async {
    final ProcessResult result = await _runFlutterTest(
        'filtering_tag', automatedTestsDirectory, flutterTestDirectory,
  ...
  testWithoutContext(
      'flutter test should run all tests when tags are unspecified', () async {
    final ProcessResult result = await _runFlutterTest(
        'filtering_tag', automatedTestsDirectory, flutterTestDirectory);
    expect(
      result,
  ...
  testWithoutContext('flutter test should run a widgetTest with a given tag',
      () async {
    final ProcessResult result = await _runFlutterTest(
  ...
  testWithoutContext(
      'flutter test should not run a widgetTest with excluded tag', () async {
    final ProcessResult result = await _runFlutterTest(
        'filtering_tag_widget', automatedTestsDirectory, flutterTestDirectory,
        extraArguments: const <String>['--exclude-tags', 'exclude-tag']);
    expect(
  ...
  testWithoutContext(
      'flutter test should run a test with an exact name in URI format',
      () async {
    final ProcessResult result = await _runFlutterTest(
        'uri_format', automatedTestsDirectory, flutterTestDirectory,
  ...
  testWithoutContext(
      'flutter test should run a test by line number in URI format', () async {
    final ProcessResult result = await _runFlutterTest(
        'uri_format', automatedTestsDirectory, flutterTestDirectory,
        query: 'line=11');
    expect(
  ...
  testWithoutContext('flutter test should test runs to completion', () async {
    ...
    expect(stdout,
        contains('test 0: Starting flutter_tester process with command'));
  ...
  testWithoutContext(
      'flutter gold skips tests where the expectations are missing', () async {
    return _testFile(
        'flutter_gold', automatedTestsDirectory, flutterTestDirectory,
        exitCode: isZero);
  });

  testWithoutContext('flutter test should respect --serve-observatory',
      () async {
    Process? process;
    ...
    try {
      ...
      sub = process.stdout.transform(utf8.decoder).listen((String e) {
        if (!completer.isCompleted && vmServiceUriRegExp.hasMatch(e)) {
          completer
              .complete(Uri.parse(vmServiceUriRegExp.firstMatch(e)!.group(0)!));
        }
      });
    ...
    try {
      ...
      sub = process.stdout.transform(utf8.decoder).listen((String e) {
        if (!completer.isCompleted && devToolsUriRegExp.hasMatch(e)) {
          completer
              .complete(Uri.parse(devToolsUriRegExp.firstMatch(e)!.group(1)!));
        }
      });
...
Future<void> _testFile(
  ...
  final String fullTestExpectation =
      fileSystem.path.join(testDirectory, '${testName}_expectation.txt');
  ...
  final List<String> expectations =
      fileSystem.file(fullTestExpectation).readAsLinesSync();
...
final RegExp _fontServerProtocolPattern =
    RegExp(r'flutter_tester.*Font server protocol version mismatch');
...
Future<ProcessResult> _runFlutterTest(
  ...
  final List<String> args = <String>[
    ...
    ...extraArguments,
    if (query != null)
      Uri.file(testPath).replace(query: query).toString()
    else
      testPath,
  ];
