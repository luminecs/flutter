...
void main() {
      ...
      for (final String line in file.readAsLinesSync()) {
        ...
        if (line.startsWith(RegExp(r'import.*commands/')) ||
          ...
          final String relativePath =
              fileSystem.path.relative(file.path, from: flutterTools);
          fail(
  ...
  test('no imports of globals without a global prefix', () {
    ...
    final Iterable<File> files = fileSystem
        .directory(fileSystem.path.join(flutterTools, 'lib', 'src'))
        .listSync(recursive: true)
        .followedBy(fileSystem
            .directory(fileSystem.path.join(
              flutterTools,
              'test',
            ))
            .listSync(recursive: true))
    ...
    for (final File file in files) {
      for (final String line in file.readAsLinesSync()) {
        if ((line.startsWith(RegExp(r'import.*globals.dart')) ||
                line.startsWith(
                    RegExp(r'import.*globals_null_migrated.dart'))) &&
  ...
  test('no unauthorized imports of dart:io', () {
    final List<String> allowedPaths = <String>[
      ...
      fileSystem.path
          .join(flutterTools, 'lib', 'src', 'base', 'error_handling_io.dart'),
      fileSystem.path.join(
          flutterTools, 'lib', 'src', 'base', 'multi_root_file_system.dart'),
    ];
        ...
        for (final String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(r'import.*dart:io')) &&
            ...
            final String relativePath =
                fileSystem.path.relative(file.path, from: flutterTools);
            fail(
                "$relativePath imports 'dart:io'; import 'lib/src/base/io.dart' instead");
          }
        }
      }
    }
  ...
  test('no unauthorized imports of package:http', () {
    final List<String> allowedPaths = <String>[
      ...
      fileSystem.path.join(
          flutterTools, 'lib', 'src', 'reporting', 'crash_reporting.dart'),
    ];
    bool isNotAllowed(FileSystemEntity entity) =>
        allowedPaths.every((String path) => path != entity.path);
        ...
        for (final String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(r'import.*package:http/')) &&
            ...
            final String relativePath =
                fileSystem.path.relative(file.path, from: flutterTools);
            fail(
                "$relativePath imports 'package:http'; import 'lib/src/base/io.dart' instead");
          }
        }
      }
    }
  ...
  test('no unauthorized imports of test_api', () {
    final List<String> allowedPaths = <String>[
      fileSystem.path
          .join(flutterTools, 'lib', 'src', 'test', 'flutter_platform.dart'),
      fileSystem.path.join(
          flutterTools, 'lib', 'src', 'test', 'flutter_web_platform.dart'),
      fileSystem.path
          .join(flutterTools, 'lib', 'src', 'test', 'test_wrapper.dart'),
    ];
    bool isNotAllowed(FileSystemEntity entity) =>
        allowedPaths.every((String path) => path != entity.path);
  ...
  test('no unauthorized imports of package:path', () {
    final List<String> allowedPath = <String>[
      ...
      fileSystem.path.join(
          flutterTools, 'test', 'general.shard', 'platform_plugins_test.dart'),
    ];
    for (final String dirName in <String>['lib', 'bin', 'test']) {
      final Iterable<File> files = fileSystem
          ...
          .where(_isDartFile)
          .where(
              (FileSystemEntity entity) => !allowedPath.contains(entity.path))
          .map(_asFile);
      for (final File file in files) {
        for (final String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(r'import.*package:path/path.dart')) &&
            ...
            final String relativePath =
                fileSystem.path.relative(file.path, from: flutterTools);
            fail(
                "$relativePath imports 'package:path/path.dart'; use 'fileSystem.path' instead");
          }
        }
      }
    }
  ...
  test('no unauthorized imports of package:file/local.dart', () {
    final List<String> allowedPath = <String>[
      fileSystem.path
          .join(flutterTools, 'test', 'integration.shard', 'test_utils.dart'),
      fileSystem.path
          .join(flutterTools, 'lib', 'src', 'base', 'file_system.dart'),
    ];
    for (final String dirName in <String>['lib', 'bin', 'test']) {
      final Iterable<File> files = fileSystem
          .directory(fileSystem.path.join(flutterTools, dirName))
        ...
        for (final String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(r'import.*dart:convert')) &&
            ...
            final String relativePath =
                fileSystem.path.relative(file.path, from: flutterTools);
            fail(
                "$relativePath imports 'dart:convert'; import 'lib/src/convert.dart' instead");
          }
        }
      }
    }
      ...
      for (final File file in files) {
        for (final String line in file.readAsLinesSync()) {
          if (line.startsWith(RegExp(
                  r'import.*package:build_runner_core/build_runner_core.dart')) ||
              line.startsWith(
                  RegExp(r'import.*package:build_runner/build_runner.dart')) ||
              line.startsWith(
                  RegExp(r'import.*package:build_config/build_config.dart')) ||
  ...
  test('no import of packages in tool_backend.dart', () {
    final File file = fileSystem
        .file(fileSystem.path.join(flutterTools, 'bin', 'tool_backend.dart'));
    for (final String line in file.readAsLinesSync()) {
      if (line.startsWith(RegExp(r'import.*package:.*'))) {
        final String relativePath =
            fileSystem.path.relative(file.path, from: flutterTools);
