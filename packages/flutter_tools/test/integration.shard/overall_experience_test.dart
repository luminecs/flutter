...
void main() {
  testWithoutContext('flutter run writes and clears pidfile appropriately',
      () async {
    ...
    final String testDirectory =
        fileSystem.path.join(flutterRoot, 'examples', 'hello_world');
    ...
    try {
      ...
      final ProcessTestResult result = await runFlutter(
        ...
        testDirectory,
        <Transition>[
          Barrier('q Quit (terminate the application on the device).',
              handler: (String line) {
      ...
      skip: Platform
          .isWindows); // [intended] Windows doesn't support sending signals so we don't care if it can store the PID.
  testWithoutContext('flutter run handle SIGUSR1/2', () async {
    ...
    final String pidFile = fileSystem.path.join(tempDirectory, 'flutter.pid');
    final String testDirectory =
        fileSystem.path.join(flutterRoot, 'dev', 'integration_tests', 'ui');
    ...
    try {
      final ProcessTestResult result = await runFlutter(
        ...
        testDirectory,
        <Transition>[
          Multiple(<Pattern>['Flutter run key commands.', 'called paint'],
              handler: (String line) {
            pid = int.parse(fileSystem.file(pidFile).readAsStringSync());
            processManager.killPid(pid, ProcessSignal.sigusr1);
            return null;
          }),
          Barrier('Performing hot reload...'.padRight(progressMessageWidth),
              logging: true),
          Multiple(<Pattern>[
            RegExp(
          ...
          Multiple(<Pattern>[
            RegExp(r'^Restarted application in .+m?s.$'),
            'called main',
            'called paint'
          ], handler: (String line) {
            return 'q';
  ...
  testWithoutContext(
      'flutter run can hot reload and hot restart, handle "p" key', () async {
    final String tempDirectory = fileSystem.systemTempDirectory
        .createTempSync('flutter_overall_experience_test.')
        .resolveSymbolicLinksSync();
    final String testDirectory =
    ...
    try {
      final ProcessTestResult result = await runFlutter(
        ...
        <Transition>[
          ...
          Barrier('Performing hot reload...'.padRight(progressMessageWidth),
              logging: true),
          Multiple(<Pattern>['ready', 'called reassemble', 'called paint'],
              handler: (String line) {
            return 'R';
          }),
          Barrier('Performing hot restart...'.padRight(progressMessageWidth)),
          Multiple(<Pattern>['ready', 'called main', 'called paint'],
              handler: (String line) {
            return 'p';
          }),
          Multiple(<Pattern>['ready', 'called paint', 'called debugPaintSize'],
              handler: (String line) {
            return 'p';
          }),
  ...
  testWithoutContext('flutter error messages include a DevTools link',
      () async {
    final String testDirectory =
        fileSystem.path.join(flutterRoot, 'dev', 'integration_tests', 'ui');
    final String tempDirectory = fileSystem.systemTempDirectory
    ...
    try {
      final ProcessTestResult result = await runFlutter(
        ...
        <Transition>[
          Barrier(RegExp(
              r'^A Dart VM Service on Flutter test device is available at: ')),
          Barrier(
              RegExp(
          ...
          Barrier('Performing hot reload...'.padRight(progressMessageWidth),
              logging: true),
          Barrier(RegExp(r'^Reloaded 0 libraries in [0-9]+ms.'),
              handler: (String line) {
            return 'q';
          }),
        ],
      ...
      expect(result.stdout, <Object>[
        ...
        matches(RegExp(
            r'^  Row .+flutter/dev/integration_tests/ui/lib/overflow\.dart:32:18$')),
        '',
  ...
  testWithoutContext('flutter run help output', () async {
    ...
    expect(
        ...
        containsAllInOrder(<Object>[
          ...
          '',
          startsWith(
              'A Dart VM Service on Flutter test device is available at: http://'),
          startsWith(
