...
class Dart2JSTarget extends Dart2WebTarget {
  ...
  Future<void> build(Environment environment) async {
    ...
    final JsCompilerConfig compilerConfig =
        JsCompilerConfig.fromBuildSystemEnvironment(environment.defines);
    final Artifacts artifacts = environment.artifacts;
    final String platformBinariesPath =
        getWebPlatformBinariesDirectory(artifacts, webRenderer).path;
...
class Dart2WasmTarget extends Dart2WebTarget {
  ...
  Future<void> build(Environment environment) async {
    ...
    final WasmCompilerConfig compilerConfig =
        WasmCompilerConfig.fromBuildSystemEnvironment(environment.defines);
    final BuildMode buildMode = BuildMode.fromCliName(buildModeEnvironment);
    final Artifacts artifacts = environment.artifacts;
    ...
    final String dartSdkPath = artifacts.getArtifactPath(
        Artifact.engineDartSdkPath,
        platform: TargetPlatform.web_javascript);
    final String dartSdkRoot =
        environment.fileSystem.directory(dartSdkPath).parent.path;

    final List<String> compilationArgs = <String>[
      artifacts.getArtifactPath(Artifact.engineDartAotRuntime,
          platform: TargetPlatform.web_javascript),
      '--disable-dart-dev',
      artifacts.getArtifactPath(Artifact.dart2wasmSnapshot,
          platform: TargetPlatform.web_javascript),
      '--packages=.dart_tool/package_config.json',
    ...
    if (compilerConfig.runWasmOpt) {
      ...
      final List<String> optimizeArgs = <String>[
        ...
        '-O3',
        '--type-merging',
        if (compilerConfig.wasmOpt == WasmOptLevel.debug) '--debuginfo',
        outputWasmFile.path,
        '-o',
      ...
      final File jsRuntimeFile =
          environment.buildDir.childFile('main.dart.unopt.mjs');
      await jsRuntimeFile
          .rename(environment.buildDir.childFile('main.dart.mjs').path);
    }
  }
  ...
  Future<void> build(Environment environment) async {
    for (final File outputFile
      ...
      final String basename =
          environment.fileSystem.path.basename(outputFile.path);
      if (shouldCopy(basename)) {
        outputFile.copySync(environment.outputDir
            .childFile(environment.fileSystem.path.basename(outputFile.path))
            .path);
      }
    }
      ...
      final File outputFile = environment.fileSystem.file(
          environment.fileSystem.path.join(
              ...
              environment.fileSystem.path
                  .relative(inputFile.path, from: webResources.path)));
...
class WebBuiltInAssets extends Target {
  const WebBuiltInAssets(this.fileSystem, this.webRenderer,
  ...
  Future<void> build(Environment environment) async {
    ...
    if (isWasm) {
      final File bootstrapFile =
          environment.outputDir.childFile('main.dart.js');
      bootstrapFile.writeAsStringSync(wasm_bootstrap
          .generateWasmBootstrapFile(webRenderer == WebRendererMode.skwasm));
    }
...
class WebServiceWorker extends Target {
  const WebServiceWorker(this.fileSystem, this.webRenderer,
      {required this.isWasm});
