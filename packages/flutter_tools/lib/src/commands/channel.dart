...
class ChannelCommand extends FlutterCommand {
  ...
  Future<void> _listChannels(
      {required bool showAll, required bool verbose}) async {
    // Beware: currentBranch could contain PII. See getBranchName().
    final String currentChannel =
        globals.flutterVersion.channel; // limited to known branch names
    assert(
    ...
    if (!currentChannelIsOfficial) {
      assert(currentChannel == kUserBranch,
  ...
  Future<void> _switchChannel(String branchName) async {
    ...
    globals
        .printStatus("Successfully switched to flutter channel '$branchName'.");
    globals.printStatus(
