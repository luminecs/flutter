...
VisualStudioValidator? get visualStudioValidator =>
    context.get<VisualStudioValidator>();
...
class VisualStudioValidator extends DoctorValidator {
  ...
  Future<ValidationResult> validate() async {
    ...
    if (_visualStudio.isInstalled) {
      ...
      messages.add(ValidationMessage(_userMessages
          .visualStudioLocation(_visualStudio.installLocation ?? 'unknown')));
      ...
      final String? windows10SdkVersion =
          _visualStudio.getWindows10SDKVersion();
      if (windows10SdkVersion != null) {
        messages.add(ValidationMessage(
            _userMessages.windows10SdkVersion(windows10SdkVersion)));
      }
      ...
      if (!_visualStudio.isAtLeastMinimumVersion) {
        ...
        status = ValidationType.partial;
        messages.add(
            ValidationMessage.error(_userMessages.visualStudioRebootRequired));
      } else if (!_visualStudio.isComplete) {
        status = ValidationType.partial;
        messages.add(
            ValidationMessage.error(_userMessages.visualStudioIsIncomplete));
      } else if (!_visualStudio.isLaunchable) {
        status = ValidationType.partial;
        messages.add(
            ValidationMessage.error(_userMessages.visualStudioNotLaunchable));
        ...
        status = ValidationType.partial;
        messages
            .add(ValidationMessage.hint(_userMessages.windows10SdkNotFound));
      }
      versionInfo =
          '${_visualStudio.displayName} ${_visualStudio.displayVersion}';
    } else {
