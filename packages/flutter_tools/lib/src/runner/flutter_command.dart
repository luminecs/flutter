...
abstract final class FlutterOptions {
  static const String kFrontendServerStarterPath =
  ...
  static const String kAssumeInitializeFromDillUpToDate =
...
abstract class FlutterCommand extends Command<void> {
  ...
  String? get packagesPath =>
      stringArg(FlutterGlobalOptions.kPackagesOption, global: true);
  ...
  int _tryParseHostVmservicePort() {
    ...
    if (vmServicePort == null && hostPort == null) {
      throwToolExit(
          'Invalid port for `--vm-service-port/--host-vmservice-port`');
    }
    try {
      return int.parse((vmServicePort ?? hostPort)!);
    } on FormatException catch (error) {
      throwToolExit(
          'Invalid port for `--vm-service-port/--host-vmservice-port`: $error');
    }
  }
  ...
  void usesWebRendererOption() {
    argParser.addOption(FlutterOptions.kWebRendererFlag,
  ...
  late final Duration? deviceDiscoveryTimeout = () {
    if ((argResults?.options.contains(FlutterOptions.kDeviceTimeout) ??
            false) &&
  ...
  DeviceConnectionInterface? get deviceConnectionInterface {
    if ((argResults?.options.contains(FlutterOptions.kDeviceConnection) ??
            false) &&
        (argResults?.wasParsed(FlutterOptions.kDeviceConnection) ?? false)) {
      final String? connectionType =
          stringArg(FlutterOptions.kDeviceConnection);
  ...
  Future<BuildInfo> getBuildInfo(
    ...
    if (argParser.options.containsKey(FlutterOptions.kWebRendererFlag)) {
      webRenderer = WebRendererMode.values
          .byName(stringArg(FlutterOptions.kWebRendererFlag)!);
    ...
    if (argParser.options.containsKey(FlutterOptions.kWebResourcesCdnFlag)) {
      final bool hasLocalWebSdk =
          argParser.options.containsKey('local-web-sdk') &&
              stringArg('local-web-sdk') != null;
      if (boolArg(FlutterOptions.kWebResourcesCdnFlag) && !hasLocalWebSdk) {
        if (!dartDefines.any((String define) =>
            define.startsWith('FLUTTER_WEB_CANVASKIT_URL='))) {
          dartDefines.add(
    ...
    return BuildInfo(
      ...
      initializeFromDill:
          argParser.options.containsKey(FlutterOptions.kInitializeFromDill)
              ...
              : null,
      assumeInitializeFromDillUpToDate: argParser.options
              .containsKey(FlutterOptions.kAssumeInitializeFromDillUpToDate) &&
  ...
  List<String> extractDartDefines(
      {required Map<String, Object?> defineConfigJsonMap}) {
  ...
  Map<String, Object?> extractDartDefineConfigJsonMap() {
    ...
    if (argParser.options
        .containsKey(FlutterOptions.kDartDefineFromFileOption)) {
      ...
      for (final String path in configFilePaths) {
        ...
        try {
          ...
          throwToolExit(
              'Json config define file "--${FlutterOptions.kDartDefineFromFileOption}=$path" format err, '
  ...
  static List<String> updateDartDefines(
      List<String> dartDefines, WebRendererMode webRenderer) {
