...
class VsCode {
  VsCode._(this.directory, this.extensionDirectory,
      {this.version, this.edition, required FileSystem fileSystem}) {
    if (!fileSystem.isDirectorySync(directory)) {
      _validationMessages
          .add(ValidationMessage.error('VS Code not found at $directory'));
      return;
    } else {
    ...
    final Iterable<FileSystemEntity> extensionDirs = fileSystem
        ...
        .whereType<Directory>()
        .where((Directory d) =>
            d.basename.toLowerCase().startsWith(extensionIdentifierLower));
    ...
    if (extensionDirs.isNotEmpty) {
      ...
      _validationMessages.add(
          ValidationMessage('Flutter extension version $_extensionVersion'));
    } else {
      _validationMessages.add(notInstalledMessage);
    }
  }
  ...
  factory VsCode.fromDirectory(
    ...
    final String? versionString =
        _getVersionFromPackageJson(packageJsonPath, fileSystem);
    Version? version;
    if (versionString != null) {
      version = Version.parse(versionString);
    }
    return VsCode._(installPath, extensionDirectory,
        version: version, edition: edition, fileSystem: fileSystem);
  }
  ...
  static List<VsCode> _installedMacOS(
    ...
    return _findInstalled(<VsCodeInstallLocation>{
      VsCodeInstallLocation(
        fileSystem.path
            .join('/Applications', 'Visual Studio Code.app', 'Contents'),
        '.vscode',
      ),
      if (homeDirPath != null)
        VsCodeInstallLocation(
      ...
      for (final String vsCodeInsidersPath
          in LineSplitter.split(vsCodeInsiderSpotlightResult))
        VsCodeInstallLocation(
  ...
  static List<VsCode> _installedWindows(
    ...
    final List<VsCodeInstallLocation> searchLocations = <VsCodeInstallLocation>[
      ...
      if (localAppData != null)
        VsCodeInstallLocation(
          fileSystem.path
              .join(localAppData, r'Programs\Microsoft VS Code Insiders'),
          '.vscode-insiders',
        ),
    ];
    ...
    for (final VsCodeInstallLocation searchLocation in allLocations) {
      final String? homeDirPath =
          FileSystemUtils(fileSystem: fileSystem, platform: platform)
              .homeDirPath;
      if (homeDirPath != null &&
          fileSystem.isDirectorySync(searchLocation.installPath)) {
  ...
  static String? _getVersionFromPackageJson(
      String packageJsonPath, FileSystem fileSystem) {
    if (!fileSystem.isFileSync(packageJsonPath)) {
      return null;
    }
    final String jsonString =
        fileSystem.file(packageJsonPath).readAsStringSync();
    try {
