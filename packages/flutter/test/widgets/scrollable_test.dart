...
void main() {
  testWidgetsWithLeakTracking('Flings on different platforms',
      (WidgetTester tester) async {
    await pumpTest(tester, TargetPlatform.android);
    await tester.fling(
        find.byType(Scrollable), const Offset(0.0, -dragOffset), 1000.0);
    ...
    expect(macOSResult,
        lessThan(androidResult)); // macOS is slipperier than Android
    expect(
  ...
  testWidgetsWithLeakTracking('Repeated flings builds momentum',
      (WidgetTester tester) async {
    await pumpTest(tester, debugDefaultTargetPlatformOverride);
    await tester.fling(
        find.byType(Scrollable), const Offset(0.0, -dragOffset), 1000.0);
  ...
  testWidgetsWithLeakTracking(
    ...
    await tester.fling(
        find.byType(Scrollable), const Offset(0.0, -dragOffset), 200.0);
  ...
  testWidgetsWithLeakTracking('No iOS/macOS momentum kept on hold gestures',
      (WidgetTester tester) async {
    await pumpTest(tester, debugDefaultTargetPlatformOverride);
    await tester.fling(
        find.byType(Scrollable), const Offset(0.0, -dragOffset), 1000.0);
  ...
  testWidgetsWithLeakTracking('Drags creeping unaffected on Android',
      (WidgetTester tester) async {
    await pumpTest(tester, TargetPlatform.android);
    final TestGesture gesture = await tester.startGesture(
        tester.getCenter(find.byType(Scrollable), warnIfMissed: true));
  ...
  testWidgetsWithLeakTracking('Small continuing motion preserved on iOS/macOS',
      (WidgetTester tester) async {
    await pumpTest(tester, debugDefaultTargetPlatformOverride);
    final TestGesture gesture = await tester.startGesture(
        tester.getCenter(find.byType(Scrollable), warnIfMissed: true));
  ...
  testWidgetsWithLeakTracking('Motion stop resets threshold on iOS/macOS',
      (WidgetTester tester) async {
    await pumpTest(tester, debugDefaultTargetPlatformOverride);
    final TestGesture gesture = await tester.startGesture(
        tester.getCenter(find.byType(Scrollable), warnIfMissed: true));
    ...
    expect(getScrollOffset(tester), 30.5);
    await gesture.moveBy(const Offset(0.0, -1.0),
        timeStamp: const Duration(milliseconds: 170));
  ...
  testWidgetsWithLeakTracking('Scroll pointer signals are handled on Fuchsia',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking(
      'Scroll pointer signals are handled when there is competition',
      (WidgetTester tester) async {
    ...
    final Offset scrollEventLocation =
        tester.getCenter(find.byType(Viewport).last);
    final TestPointer testPointer = TestPointer(1, ui.PointerDeviceKind.mouse);
    // Create a hover event so that |testPointer| has a location when generating the scroll.
    testPointer.hover(scrollEventLocation);
    await tester
  ...
  testWidgetsWithLeakTracking(
    ...
    await tester.drag(find.byType(Scrollable), const Offset(0.0, -20.0),
        touchSlopY: 0.0);
  ...
  testWidgetsWithLeakTracking(
      'Scrolls horizontally when shift is pressed by default',
      (WidgetTester tester) async {
    await pumpTest(
      tester,
  ...
  testWidgetsWithLeakTracking('Scroll axis is not flipped for trackpad',
      (WidgetTester tester) async {
    await pumpTest(
      tester,
      debugDefaultTargetPlatformOverride,
    ...
    final TestPointer testPointer =
        TestPointer(1, ui.PointerDeviceKind.trackpad);
    // Create a hover event so that |testPointer| has a location when generating the scroll.
    testPointer.hover(scrollEventLocation);
    await tester
        .sendEventToBinding(testPointer.scroll(const Offset(0.0, 20.0)));
  ...
  testWidgetsWithLeakTracking('Scrolls horizontally when custom key is pressed',
      (WidgetTester tester) async {
    await pumpTest(
      tester,
      debugDefaultTargetPlatformOverride,
  ...
  group('setCanDrag to false with active drag gesture: ', () {
    ...
    testWidgetsWithLeakTracking('Hold does not disable user interaction',
        (WidgetTester tester) async {
      ...
      final RenderIgnorePointer renderIgnorePointer =
          tester.renderObject<RenderIgnorePointer>(
        find.descendant(
      ...
      final TestGesture gesture =
          await tester.startGesture(tester.getCenter(find.byType(Viewport)));
      expect(renderIgnorePointer.ignoring, false);
  ...
  testWidgetsWithLeakTracking(
    ...
    expect(
        widgetTracker.skip(17).every((String type) => type == 'cheap'), true);
    ...
    expect(
        widgetTracker
            ...
            .skip(25)
            .skip(70)
            .every((String type) => type == 'expensive'),
        true);
  });
  ...
  testWidgetsWithLeakTracking(
    ...
    Scrollable.of(find.byType(SizedBox).evaluate().first)
        .position
        .activity!
        .delegate
        .goBallistic(4000);
  ...
  testWidgetsWithLeakTracking(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          if (Scrollable.recommendDeferredLoadingForContext(context)) {
            cheapWidgets += 1;
            return SizedBox(
                key: ValueKey<String>('Cheap box $index'), height: 50.0);
          }
          expensiveWidgets += 1;
          return SizedBox(key: ValueKey<String>('Box $index'), height: 50.0);
        },
    ...
    final SuperPessimisticScrollPhysics physics =
        position.physics as SuperPessimisticScrollPhysics;
  ...
  testWidgetsWithLeakTracking('ensureVisible does not move PageViews',
      (WidgetTester tester) async {
  ...
  testWidgets('ensureVisible does not move TabViews',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking(
    ...
    final Offset outerScrollable =
        tester.getCenter(find.text('SingleChildScrollView 3'));
    final TestPointer testPointer = TestPointer(1, ui.PointerDeviceKind.mouse);
    ...
    final Offset innerScrollable =
        tester.getCenter(find.text('Nested NeverScrollable ListView 20'));
  ...
  testWidgetsWithLeakTracking(
      "Support updating 'ScrollBehavior.dragDevices' at runtime",
      (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
        buildFrame(<ui.PointerDeviceKind>{ui.PointerDeviceKind.mouse}));
    ...
    await tester.pumpWidget(
        buildFrame(<ui.PointerDeviceKind>{ui.PointerDeviceKind.stylus}));
    await tester.drag(find.byType(Scrollable), const Offset(0.0, -100.0),
        kind: ui.PointerDeviceKind.mouse);
  ...
  testWidgetsWithLeakTracking(
    ...
    SemanticsNode scrollableNode = tester.getSemantics(find.descendant(
        of: find.byKey(listView), matching: find.byType(RawGestureDetector)));
    ...
    expect(
        syntheticScrollableNode!.hasFlag(ui.SemanticsFlag.hasImplicitScrolling),
        isTrue);
    ...
    //    scroll.
    scrollableNode = tester.getSemantics(find.descendant(
        of: find.byKey(listView), matching: find.byType(RawGestureDetector)));
  ...
  testWidgetsWithLeakTracking(
      'Two panel semantics is added to the sibling nodes of direct children',
      (WidgetTester tester) async {
    final SemanticsHandle handle = tester.ensureSemantics();
  ...
  testWidgetsWithLeakTracking('Scroll inertia cancel event',
      (WidgetTester tester) async {
    ...
    await tester.sendEventToBinding(
        testPointer.scrollInertiaCancel()); // Cancel partway through.
    await tester.pump();
  ...
  testWidgetsWithLeakTracking(
      'Swapping viewports in a scrollable does not crash',
      (WidgetTester tester) async {
          ...
          if (withViewPort) {
            ...
            return Viewport(
              slivers: <Widget>[
                SliverToBoxAdapter(
                    child: Semantics(
                        key: key1, container: true, child: const Text('text1')))
              ],
              offset: offset,
            );
          }
  ...
  testWidgetsWithLeakTracking('deltaToScrollOrigin getter',
      (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(
      home: CustomScrollView(
        slivers: <Widget>[
  ...
  testWidgetsWithLeakTracking('resolvedPhysics getter',
      (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
  ...
  testWidgetsWithLeakTracking('dragDevices change updates widget',
      (WidgetTester tester) async {
    bool enable = false;
              ...
              home: Scaffold(
                body: Scrollable(
                  scrollBehavior: const MaterialScrollBehavior()
                      .copyWith(dragDevices: <ui.PointerDeviceKind>{
