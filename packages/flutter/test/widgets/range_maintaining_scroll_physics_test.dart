...
class ExpandingBox extends StatefulWidget {
  const ExpandingBox(
      {super.key, required this.collapsedSize, required this.expandedSize});
...
class _ExpandingBoxState extends State<ExpandingBox>
  ...
  void toggleSize() {
    setState(() {
      _height = _height == widget.collapsedSize
          ? widget.expandedSize
...
void main() {
  testWidgetsWithLeakTracking('shrink listview', (WidgetTester tester) async {
    ...
    final TestGesture drag1 =
        await tester.startGesture(const Offset(10.0, 500.0));
    await tester.pump();
  ...
  testWidgetsWithLeakTracking('shrink listview while dragging',
    ...
    expect(await tester.pumpAndSettle(),
        2); // Nothing to animate, only one semantics update
    await drag1.up();
    expect(await tester.pumpAndSettle(), 1); // Nothing to animate
    expect(position.pixels, moreOrLessEquals(500.0));
    expect(position.minScrollExtent, 0.0);
  ...
  testWidgetsWithLeakTracking('shrink listview while ballistic',
      (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
    ...
    expect(position.pixels, 0.0);
    await drag1.moveTo(
        const Offset(10.0, 50.0)); // to get past the slop and trigger the drag
  ...
  testWidgetsWithLeakTracking('expanding page views',
      (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
        const Padding(padding: EdgeInsets.zero, child: TabBarDemo()));
            ...
            child: SizedBox(
              ...
              child: ListView(
                children: const <Widget>[
                  SizedBox(height: 100.0, child: Placeholder())
                ],
              ),
            ),
          ),
