...
void main() {
  testWidgetsWithLeakTracking(
      'Scrolling with list view changes, leaving the overscroll',
  ...
  testWidgetsWithLeakTracking(
      'Ability to keep a PageView at the end manually (issue 62209)',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking(
    ...
    final TestGesture touchGesture = await tester.createGesture(
        kind:
            PointerDeviceKind.touch); // ignore: avoid_redundant_argument_values
    ...
    expect(controller.position.activity?.shouldIgnorePointer,
        isTrue); // Pointer is ignored for touch scrolling.
    ...
    expect(controller.position.activity?.shouldIgnorePointer,
        isTrue); // Pointer is ignored following touch scrolling.
    ...
    controller.jumpTo(0);
    await tester.pump();
    final TestGesture trackpadGesture =
    ...
    await tester.pump();
    await trackpadGesture.panZoomUpdate(tester.getCenter(find.byType(ListView)),
    ...
    await tester.pumpAndSettle();
    expect(controller.position.activity?.shouldIgnorePointer,
        isFalse); // Pointer is not ignored for trackpad scrolling.
    ...
    await tester.trackpadFling(
        find.byType(ListView), const Offset(0, -200), 1000);
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 100));
    expect(controller.position.activity?.shouldIgnorePointer,
