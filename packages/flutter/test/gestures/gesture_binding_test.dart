...
class TestGestureFlutterBinding extends BindingBase
  ...
  static TestGestureFlutterBinding get instance =>
...
void main() {
  final TestGestureFlutterBinding binding =
  ...
  test('Pointer hover events', () {
    ...
    GestureBinding.instance.pointerRouter
        .addGlobalRoute(pointerRouterEvents.add);
  ...
  test('Can expand add and hover pointers', () {
    ...
    final List<PointerEvent> events = PointerEventConverter.expand(
        packet.data, (int viewId) => devicePixelRatio).toList();
  ...
  test('Can handle malformed scrolling event.', () {
    ...
    events =
        PointerEventConverter.expand(packet.data, (int viewId) => 0).toList();
    expect(events.length, 0);
  });
  ...
  test('Should not synthesize kPrimaryButton for mouse', () {
    ...
    for (final PointerDeviceKind kind in <PointerDeviceKind>[
      ...
      expect(events[3], isA<PointerMoveEvent>());
      expect(events[3].buttons,
          equals(kMiddleMouseButton | kSecondaryMouseButton));
      expect(events[4], isA<PointerUpEvent>());
      expect(events[4].buttons, equals(0));
    }
  });
  ...
  test('PointerEventConverter processes view IDs', () {
    ...
    const List<ui.PointerData> data = <ui.PointerData>[
      ui.PointerData(
          viewId: startID + 0,
          change: ui
              .PointerChange.cancel), // ignore: avoid_redundant_argument_values
      ...
      ui.PointerData(
          viewId: startID + 7, change: ui.PointerChange.panZoomStart),
      ui.PointerData(
          viewId: startID + 8, change: ui.PointerChange.panZoomUpdate),
    ...
    final List<int> expectedViewIds =
        List<int>.generate(10, (int index) => startID + index);
