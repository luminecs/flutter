...
void main() {
  ...
  test('computed hit slop values are based on pointer device kind', () {
    expect(
        computeHitSlop(PointerDeviceKind.mouse, null), kPrecisePointerHitSlop);
    ...
    expect(
        computePanSlop(PointerDeviceKind.mouse, null), kPrecisePointerPanSlop);
  ...
  test(
      'computed hit slop values defer to device value when pointer kind is touch',
      () {
    const DeviceGestureSettings settings = DeviceGestureSettings(touchSlop: 1);

    expect(computeHitSlop(PointerDeviceKind.mouse, settings),
    ...
    expect(computePanSlop(PointerDeviceKind.mouse, settings),
  ...
  test('transformPosition', () {
    ...
    expect(
        PointerEvent.transformPosition(Matrix4.identity(), position), position);
    final Matrix4 transform = Matrix4.translationValues(10, 20, 0);
    expect(PointerEvent.transformPosition(transform, position),
  ...
  test('transforming events', () {
    final Matrix4 transform = (Matrix4.identity()..scale(2.0, 2.0, 1.0))
        .multiplied(Matrix4.translationValues(10.0, 20.0, 0.0));
  ...
  group('copyWith', () {
    ...
    test('PointerAddedEvent.copyWith()', () {
      final PointerEvent event =
          added.copyWith(position: position, timeStamp: timeStamp);
    ...
    test('PointerHoverEvent.copyWith()', () {
      final PointerEvent event =
          hover.copyWith(position: position, timeStamp: timeStamp);
    ...
    test('PointerDownEvent.copyWith()', () {
      final PointerEvent event =
          down.copyWith(position: position, timeStamp: timeStamp);
    ...
    test('PointerMoveEvent.copyWith()', () {
      final PointerEvent event =
          move.copyWith(position: position, timeStamp: timeStamp);
    ...
    test('PointerUpEvent.copyWith()', () {
      final PointerEvent event =
          up.copyWith(position: position, timeStamp: timeStamp);
    ...
    test('PointerRemovedEvent.copyWith()', () {
      final PointerEvent event =
          removed.copyWith(position: position, timeStamp: timeStamp);
  ...
  test('Ensure certain event types are allowed', () {
    ...
    expect(
        const PointerHoverEvent(kind: PointerDeviceKind.trackpad), isNotNull);
    // Regression test for https://github.com/flutter/flutter/issues/108176
    expect(
        const PointerScrollInertiaCancelEvent(kind: PointerDeviceKind.trackpad),
        isNotNull);

    expect(
        const PointerScrollEvent(kind: PointerDeviceKind.trackpad), isNotNull);
    // The test passes if it compiles.
  });

  test('Ensure certain event types are not allowed', () {
    expect(() => PointerDownEvent(kind: PointerDeviceKind.trackpad),
        throwsAssertionError);
    expect(() => PointerMoveEvent(kind: PointerDeviceKind.trackpad),
        throwsAssertionError);
    expect(() => PointerUpEvent(kind: PointerDeviceKind.trackpad),
        throwsAssertionError);
  });
}
