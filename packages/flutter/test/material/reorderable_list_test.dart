...
void main() {
  group('$ReorderableListView', () {
    ...
    group('in vertical mode', () {
      testWidgetsWithLeakTracking(
        ...
        final List<String> currentOriginalListItems =
            originalListItems.take(1).toList();
      ...
      testWidgetsWithLeakTracking(
        ...
        expect(listItems,
            orderedEquals(<String>['Item 2', 'Item 3', 'Item 1', 'Item 4']));
      });
      ...
      testWidgetsWithLeakTracking(
        ...
        await longPressDrag(
          ...
          tester.getCenter(find.text('Item 4')) +
              const Offset(0.0, itemHeight * 2),
        );
        await tester.pumpAndSettle();
        expect(listItems,
            orderedEquals(<String>['Item 2', 'Item 3', 'Item 4', 'Item 1']));
      });
      ...
      testWidgetsWithLeakTracking(
        ...
        await tester.pumpAndSettle();
        expect(listItems,
            orderedEquals(<String>['Item 4', 'Item 1', 'Item 2', 'Item 3']));
      });
      ...
      testWidgetsWithLeakTracking(
        ...
        await tester.pumpAndSettle();
        expect(listItems,
            orderedEquals(<String>['Item 1', 'Item 3', 'Item 2', 'Item 4']));
      });

      testWidgetsWithLeakTracking('properly reorders with a header',
          (WidgetTester tester) async {
      ...
      testWidgetsWithLeakTracking('properly reorders with a footer',
          (WidgetTester tester) async {
      ...
      testWidgetsWithLeakTracking(
        ...
        double getListHeight() {
          final RenderSliverList listScrollView =
              tester.renderObject(find.byType(SliverList));
        ...
        TestGesture drag = await tester
            .startGesture(tester.getCenter(find.text('Normal item')));
        await tester.pump(kLongPressTimeout + kPressTimeout);
        await tester.pumpAndSettle();
        expect(getListHeight(), kDraggingListHeight);
        ...
        // Drag a tall item
        drag =
            await tester.startGesture(tester.getCenter(find.text('Tall item')));
        await tester.pump(kLongPressTimeout + kPressTimeout);
        await tester.pumpAndSettle();
        expect(getListHeight(), kDraggingListHeight);
        // Move it
      ...
      testWidgetsWithLeakTracking('Vertical drag in progress golden image',
          (WidgetTester tester) async {
        ...
        final TestGesture drag = await tester
            .startGesture(tester.getCenter(find.byKey(const Key('blue'))));
        await tester.pump(kLongPressTimeout + kPressTimeout);
        ...
        _StatefulState findState(Key key) {
          return find
              .byElementPredicate((Element element) =>
                  element.findAncestorWidgetOfExactType<_Stateful>()?.key ==
                  key)
              .evaluate()
              .first
      ...
      testWidgetsWithLeakTracking('Preserves children states when rebuilt',
          (WidgetTester tester) async {
      ...
      testWidgetsWithLeakTracking(
          'Uses the PrimaryScrollController when available',
          (WidgetTester tester) async {
        ...
        final Widget reorderableList = ReorderableListView(
          children: const <Widget>[
            SizedBox(
                width: 100.0, height: 100.0, key: Key('C'), child: Text('C')),
            SizedBox(
                width: 100.0, height: 100.0, key: Key('B'), child: Text('B')),
            SizedBox(
                width: 100.0, height: 100.0, key: Key('A'), child: Text('A')),
          ],
          onReorder: (int oldIndex, int newIndex) {},
        );
      ...
      testWidgetsWithLeakTracking(
          'ReorderableList auto scrolling is fast enough',
          (WidgetTester tester) async {
        ...
        final Offset bottomRight =
            tester.getBottomRight(find.byType(ReorderableListView));
        ...
        expect(controller.offset,
            greaterThan(kMinimumAllowedAutoScrollDistancePer5ms));
        await tester.pump(const Duration(milliseconds: 5));
        expect(controller.offset,
            greaterThan(kMinimumAllowedAutoScrollDistancePer5ms * 2));
        await tester.pump(const Duration(milliseconds: 5));
        expect(controller.offset,
            greaterThan(kMinimumAllowedAutoScrollDistancePer5ms * 3));
        await tester.pump(const Duration(milliseconds: 5));
        expect(controller.offset,
            greaterThan(kMinimumAllowedAutoScrollDistancePer5ms * 4));
      });
      ...
      testWidgetsWithLeakTracking(
        ...
        final Widget overlay = Overlay(
          initialEntries: <OverlayEntry>[
            entry = OverlayEntry(
                builder: (BuildContext context) => reorderableList),
          ],
        );
      ...
      group('Accessibility (a11y/Semantics)', () {
        Map<CustomSemanticsAction, VoidCallback> getSemanticsActions(
            int index) {
        ...
        const CustomSemanticsAction moveToStart =
            CustomSemanticsAction(label: 'Move to the start');
        const CustomSemanticsAction moveToEnd =
            CustomSemanticsAction(label: 'Move to the end');
        const CustomSemanticsAction moveUp =
            CustomSemanticsAction(label: 'Move up');
        const CustomSemanticsAction moveDown =
            CustomSemanticsAction(label: 'Move down');

        testWidgetsWithLeakTracking(
            'Provides the correct accessibility actions in LTR and RTL modes',
            (WidgetTester tester) async {
          ...
          for (final TextDirection direction in TextDirection.values) {
            ...
            final Map<CustomSemanticsAction, VoidCallback>
                firstSemanticsActions = getSemanticsActions(0);
            expect(firstSemanticsActions.length, 2,
                reason:
                    'The first list item should have 2 custom actions with $direction.');
            expect(firstSemanticsActions.containsKey(moveToStart), false,
                reason:
                    'The first item cannot `Move to the start` with $direction.');
            ...
            expect(firstSemanticsActions.containsKey(moveDown), true,
                reason:
                    'The first item should be able to `Move down` with $direction.');
            expect(firstSemanticsActions.containsKey(moveToEnd), true,
                reason:
                    'The first item should be able to `Move to the end` with $direction.');

            // Items in the middle can be moved to the start, end, up or down.
            for (int i = 1; i < listItems.length - 1; i += 1) {
              final Map<CustomSemanticsAction, VoidCallback>
                  ithSemanticsActions = getSemanticsActions(i);
              expect(ithSemanticsActions.length, 4,
                  reason:
                      'List item $i should have 4 custom actions with $direction.');
              expect(ithSemanticsActions.containsKey(moveToStart), true,
                  reason:
                      'List item $i should be able to `Move to the start` with $direction.');
              expect(ithSemanticsActions.containsKey(moveUp), true,
                  reason:
                      'List item $i should be able to `Move up` with $direction.');
              expect(ithSemanticsActions.containsKey(moveDown), true,
                  reason:
                      'List item $i should be able to `Move down` with $direction.');
              expect(ithSemanticsActions.containsKey(moveToEnd), true,
                  reason:
                      'List item $i should be able to `Move to the end` with $direction.');
            }
            ...
            expect(lastSemanticsActions.length, 2,
                reason:
                    'The last list item should have 2 custom actions with $direction.');
            expect(lastSemanticsActions.containsKey(moveToStart), true,
                reason:
                    'The last item should be able to `Move to the start` with $direction.');
            expect(lastSemanticsActions.containsKey(moveUp), true,
                reason:
                    'The last item should be able to `Move up` with $direction.');
            ...
            expect(lastSemanticsActions.containsKey(moveToEnd), false,
                reason:
                    'The last item cannot `Move to the end` with $direction.');
          }
          handle.dispose();
        });

        testWidgetsWithLeakTracking(
            'First item accessibility (a11y) actions work',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
          ...
          await tester.pumpWidget(build());
          Map<CustomSemanticsAction, VoidCallback> firstSemanticsActions =
          ...
          await tester.pumpAndSettle();
          expect(listItems,
              orderedEquals(<String>['Item 3', 'Item 2', 'Item 4', 'Item 1']));

          handle.dispose();
        });

        testWidgetsWithLeakTracking(
            'Middle item accessibility (a11y) actions work',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
          ...
          await tester.pumpWidget(build());
          Map<CustomSemanticsAction, VoidCallback> middleSemanticsActions =
              getSemanticsActions(1);
          middleSemanticsActions[moveToEnd]!();
          await tester.pumpAndSettle();
          expect(listItems,
              orderedEquals(<String>['Item 1', 'Item 3', 'Item 4', 'Item 2']));
          ...
          await tester.pumpAndSettle();
          expect(listItems,
              orderedEquals(<String>['Item 1', 'Item 4', 'Item 3', 'Item 2']));
          ...
          await tester.pumpAndSettle();
          expect(listItems,
              orderedEquals(<String>['Item 4', 'Item 1', 'Item 3', 'Item 2']));

          handle.dispose();
        });

        testWidgetsWithLeakTracking(
            'Last item accessibility (a11y) actions work',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
          ...
          await tester.pumpWidget(build());
          Map<CustomSemanticsAction, VoidCallback> lastSemanticsActions =
              getSemanticsActions(listItems.length - 1);
        ...
        testWidgetsWithLeakTracking(
          ...
          final SemanticsNode semanticsNode =
              tester.getSemantics(find.byKey(const Key('Switch tile')));
    ...
    group('in horizontal mode', () {
      ...
      testWidgetsWithLeakTracking('properly reorders with a header',
        ...
        expect(find.text('Header Text'), findsOneWidget);
        expect(listItems,
            orderedEquals(<String>['Item 2', 'Item 4', 'Item 3', 'Item 1']));
      });
      ...
      testWidgetsWithLeakTracking('Horizontal drag in progress golden image',
          (WidgetTester tester) async {
        debugDisableShadows = false;
        final Widget reorderableListView = ReorderableListView(
          scrollDirection: Axis.horizontal,
      ...
      group('Accessibility (a11y/Semantics)', () {
        ...
        const CustomSemanticsAction moveToEnd =
            CustomSemanticsAction(label: 'Move to the end');
        const CustomSemanticsAction moveLeft =
            CustomSemanticsAction(label: 'Move left');
        const CustomSemanticsAction moveRight =
            CustomSemanticsAction(label: 'Move right');

        testWidgetsWithLeakTracking(
            'Provides the correct accessibility actions in LTR mode',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          ...
          expect(firstSemanticsActions.containsKey(moveLeft), false,
          ...
          expect(lastSemanticsActions.containsKey(moveRight), false,
        ...
        testWidgetsWithLeakTracking(
            'Provides the correct accessibility actions in Right-To-Left directionality',
            (WidgetTester tester) async {
          ...
          expect(firstSemanticsActions.containsKey(moveToStart), false,
              reason: 'The first item cannot `Move to the start`.');
          expect(firstSemanticsActions.containsKey(moveRight), false,
          ...
          expect(lastSemanticsActions.containsKey(moveLeft), false,
        ...
        testWidgetsWithLeakTracking(
            'First item accessibility (a11y) actions work in LTR mode',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
        ...
        testWidgetsWithLeakTracking(
            'Middle item accessibility (a11y) actions work in LTR mode',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
        ...
        testWidgetsWithLeakTracking(
            'Last item accessibility (a11y) actions work in LTR mode',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
        ...
        testWidgetsWithLeakTracking(
            'Last item accessibility (a11y) actions work in Right-To-Left directionality',
            (WidgetTester tester) async {
          // In RTL mode, the right is the start and the left is the end.
          // The array representation is unchanged (LTR), but the direction of the motion actions is reversed.
          final SemanticsHandle handle = tester.ensureSemantics();
    ...
    testWidgetsWithLeakTracking(
        'ReorderableListView.builder asserts on negative childCount',
        (WidgetTester tester) async {
      expect(
    ...
    testWidgetsWithLeakTracking(
        'ReorderableListView.builder only creates the children it needs',
        (WidgetTester tester) async {
    ...
    group('Padding', () {
      testWidgetsWithLeakTracking('Padding with no header & footer',
          (WidgetTester tester) async {
        const EdgeInsets padding = EdgeInsets.fromLTRB(10, 20, 30, 40);

        // Vertical
        await tester.pumpWidget(build(padding: padding));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
            const Rect.fromLTRB(10, 20, 770, 68));
        expect(tester.getRect(find.byKey(const Key('Item 4'))),
            const Rect.fromLTRB(10, 164, 770, 212));

        // Horizontal
        await tester.pumpWidget(
            build(padding: padding, scrollDirection: Axis.horizontal));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
      ...
      testWidgetsWithLeakTracking('Padding with header or footer',
          (WidgetTester tester) async {
        const EdgeInsets padding = EdgeInsets.fromLTRB(10, 20, 30, 40);
        ...
        // Vertical Header
        await tester
            .pumpWidget(build(padding: padding, header: verticalHeader));
        expect(tester.getRect(find.byKey(headerKey)),
            const Rect.fromLTRB(10, 20, 770, 30));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
        ...
        // Vertical Footer
        await tester
            .pumpWidget(build(padding: padding, footer: verticalFooter));
        expect(tester.getRect(find.byKey(footerKey)),
            const Rect.fromLTRB(10, 212, 770, 222));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
            const Rect.fromLTRB(10, 20, 770, 68));
        expect(tester.getRect(find.byKey(const Key('Item 4'))),
            const Rect.fromLTRB(10, 164, 770, 212));
        ...
        await tester.pumpWidget(
            build(padding: padding, header: verticalHeader, reverse: true));
        expect(tester.getRect(find.byKey(headerKey)),
            const Rect.fromLTRB(10, 550, 770, 560));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
        ...
        await tester.pumpWidget(
            build(padding: padding, footer: verticalFooter, reverse: true));
        expect(tester.getRect(find.byKey(footerKey)),
            const Rect.fromLTRB(10, 358, 770, 368));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
        ...
        await tester.pumpWidget(build(
            ...
            scrollDirection: Axis.horizontal));
        expect(tester.getRect(find.byKey(headerKey)),
            const Rect.fromLTRB(10, 20, 20, 560));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
        ...
        await tester.pumpWidget(build(
            ...
            scrollDirection: Axis.horizontal));
        expect(tester.getRect(find.byKey(footerKey)),
            const Rect.fromLTRB(202, 20, 212, 560));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
            const Rect.fromLTRB(10, 20, 58, 560));
        expect(tester.getRect(find.byKey(const Key('Item 4'))),
            const Rect.fromLTRB(154, 20, 202, 560));
        ...
        await tester.pumpWidget(build(
            ...
            reverse: true));
        expect(tester.getRect(find.byKey(headerKey)),
            const Rect.fromLTRB(760, 20, 770, 560));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
        ...
        await tester.pumpWidget(build(
            ...
            reverse: true));
        expect(tester.getRect(find.byKey(footerKey)),
            const Rect.fromLTRB(568, 20, 578, 560));
        expect(tester.getRect(find.byKey(const Key('Item 1'))),
    ...
    testWidgetsWithLeakTracking('ReorderableListView can be reversed',
        (WidgetTester tester) async {
      final Widget reorderableListView = ReorderableListView(
        reverse: true,
        onReorder: (int oldIndex, int newIndex) {},
      ...
      expect(tester.getCenter(find.text('A')).dy,
          greaterThan(tester.getCenter(find.text('B')).dy));
    });

    testWidgetsWithLeakTracking('Animation test when placing an item in place',
        (WidgetTester tester) async {
    ...
    testWidgetsWithLeakTracking(
      ...
      expect(
          find.byType(ReorderableDelayedDragStartListener), findsNWidgets(4));
    ...
    testWidgetsWithLeakTracking(
      ...
      expect(tester.getTopRight(dragHandle).dx,
          tester.getSize(listView).width - 8);
    });
    ...
    testWidgetsWithLeakTracking(
        ...
        (WidgetTester tester) async {
      await tester.pumpWidget(build(
          scrollDirection: Axis.horizontal, platform: TargetPlatform.macOS));
      final Finder listView = find.byType(ReorderableListView);
      final Finder item1 = find.byKey(const Key('Item 1'));
      final Finder dragHandle = find.byIcon(Icons.drag_handle).first;
      ...
      expect(tester.getBottomRight(dragHandle).dy,
          tester.getSize(listView).height - 8);
    });
  });
  ...
  testWidgetsWithLeakTracking(
    ...
    expect(exception.toString(),
        contains('Every item of ReorderableListView must have a key.'));
  });

  testWidgetsWithLeakTracking('Throws an error if no overlay present',
      (WidgetTester tester) async {
    final Widget reorderableList = ReorderableListView(
      children: const <Widget>[
    ...
    expect(
        exception.toString(),
        contains(
            'ReorderableListView widgets require an Overlay widget ancestor'));
  });
  ...
  testWidgetsWithLeakTracking(
      'ReorderableListView.builder asserts on both non-null itemExtent and prototypeItem',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking(
      'ReorderableListView auto scrolls speed is configurable',
      (WidgetTester tester) async {
    Future<void> pumpFor({
