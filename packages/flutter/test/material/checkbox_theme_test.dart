...
void main() {
  test('CheckboxThemeData copyWith, ==, hashCode basics', () {
    ...
    expect(const CheckboxThemeData().hashCode,
  ...
  test('CheckboxThemeData defaults', () {
    ...
    const CheckboxTheme theme =
        CheckboxTheme(data: CheckboxThemeData(), child: SizedBox());
  ...
  testWidgetsWithLeakTracking('Default CheckboxThemeData debugFillProperties',
  ...
  testWidgetsWithLeakTracking('Checkbox is themeable',
      (WidgetTester tester) async {
    ...
    const VisualDensity visualDensity =
        VisualDensity(vertical: 1.0, horizontal: 1.0);
      ...
      return MaterialApp(
        theme: ThemeData(
          checkboxTheme: CheckboxThemeData(
            mouseCursor:
                const MaterialStatePropertyAll<MouseCursor?>(mouseCursor),
            fillColor:
    ...
    expect(tester.getSize(find.byType(Checkbox)),
    ...
    expect(
        _getCheckboxMaterial(tester), paints..circle(color: hoverOverlayColor));
  ...
  testWidgetsWithLeakTracking(
      'Checkbox properties are taken over the theme values',
      (WidgetTester tester) async {
    tester.binding.focusManager.highlightStrategy =
        FocusHighlightStrategy.alwaysTraditional;
  ...
  testWidgetsWithLeakTracking(
      'Checkbox activeColor property is taken over the theme',
      (WidgetTester tester) async {
    const Color themeSelectedFillColor = Color(0xfffffff1);
    const Color themeDefaultFillColor = Color(0xfffffff0);
    const Color selectedFillColor = Color(0xfffffff6);
  ...
  testWidgetsWithLeakTracking(
    ...
    await tester.pumpAndSettle();
    expect(
        _getCheckboxMaterial(tester), paints..path(color: localThemeFillColor));
    expect(
        _getCheckboxMaterial(tester),
        paints
  ...
  test('CheckboxThemeData lerp from populated to null parameters', () {
    ...
    expect(
        lerped.fillColor!.resolve(<MaterialState>{}), const Color(0x80fffff0));
    expect(
        lerped.checkColor!.resolve(<MaterialState>{}), const Color(0x80fffff1));
    expect(lerped.overlayColor!.resolve(<MaterialState>{}),
        const Color(0x80fffff2));
  ...
  test('CheckboxThemeData lerp from populated parameters', () {
    ...
    final CheckboxThemeData lerped =
        CheckboxThemeData.lerp(themeA, themeB, 0.5);

    expect(
        lerped.fillColor!.resolve(<MaterialState>{}), const Color(0xfffffff1));
    expect(
        lerped.checkColor!.resolve(<MaterialState>{}), const Color(0xfffffff2));
    expect(lerped.overlayColor!.resolve(<MaterialState>{}),
        const Color(0xfffffff3));
    expect(lerped.splashRadius, 6);
    expect(lerped.materialTapTargetSize, MaterialTapTargetSize.shrinkWrap);
    expect(lerped.visualDensity,
