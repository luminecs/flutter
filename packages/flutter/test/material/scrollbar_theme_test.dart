...
const Rect _kMaterialDesignInitialThumbRect =
    Rect.fromLTRB(790.0, 0.0, 798.0, 90.0);
...
void main() {
  test('ScrollbarThemeData copyWith, ==, hashCode basics', () {
    ...
    expect(const ScrollbarThemeData().hashCode,
  ...
  testWidgetsWithLeakTracking('ScrollbarTheme can disable gestures',
      (WidgetTester tester) async {
    final ScrollController scrollController = ScrollController();
    await tester.pumpWidget(MaterialApp(
      theme: ThemeData(
  ...
  testWidgetsWithLeakTracking(
    ...
    (WidgetTester tester) async {
      ...
      final (ScrollController controller1, Widget frame1) =
          buildFrame(ThemeData(
        colorScheme: const ColorScheme.light(),
      ));
      ...
      // Drag scrollbar behavior
      const double scrollAmount = 10.0;
      TestGesture dragScrollbarGesture =
          await tester.startGesture(const Offset(797.0, 45.0));
      await tester.pumpAndSettle();
      ...
      final (ScrollController controller2, Widget frame2) =
          buildFrame(ThemeData(
        ...
        MaterialApp(
          theme: ThemeData(useMaterial3: false).copyWith(
            scrollbarTheme: _scrollbarTheme(
              trackVisibility:
                  MaterialStateProperty.resolveWith(getTrackVisibility),
            ),
          ),
          home: ScrollConfiguration(
            behavior: const NoScrollbarBehavior(),
  ...
  testWidgetsWithLeakTracking('Default ScrollbarTheme debugFillProperties',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking('ScrollbarTheme implements debugFillProperties',
      (WidgetTester tester) async {
    final DiagnosticPropertiesBuilder builder = DiagnosticPropertiesBuilder();
    ScrollbarThemeData(
...
ScrollbarThemeData _scrollbarTheme({
  ...
  return ScrollbarThemeData(
    ...
    trackBorderColor: trackBorderColor ??
        MaterialStateProperty.resolveWith(_getTrackBorderColor),
