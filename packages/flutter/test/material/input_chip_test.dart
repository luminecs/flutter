...
void main() {
  testWidgetsWithLeakTracking('InputChip.color resolves material states',
  ...
  testWidgetsWithLeakTracking('InputChip uses provided state color properties',
      (WidgetTester tester) async {
    const Color disabledColor = Color(0xff00ff00);
    const Color backgroundColor = Color(0xff0000ff);
    const Color selectedColor = Color(0xffff0000);
  ...
  testWidgetsWithLeakTracking('InputChip can be tapped',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Material(
  ...
  testWidgetsWithLeakTracking('loses focus when disabled',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking('cannot be traversed to when disabled',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking(
      'Input chip has correct selected color when enabled - M3 defaults',
      (WidgetTester tester) async {
    final ChipThemeData material3ChipDefaults =
        ThemeData(useMaterial3: true).chipTheme;
    await pumpCheckmarkChip(
      tester,
    ...
    expect(materialBox,
        paints..rrect(color: material3ChipDefaults.backgroundColor));
  });

  testWidgetsWithLeakTracking(
      'Input chip has correct selected color when disabled - M3 defaults',
      (WidgetTester tester) async {
    final ChipThemeData material3ChipDefaults =
        ThemeData(useMaterial3: true).chipTheme;
    await pumpCheckmarkChip(
    ...
    final RenderBox materialBox = getMaterialBox(tester);
    expect(
        materialBox, paints..path(color: material3ChipDefaults.disabledColor));
  });
}
