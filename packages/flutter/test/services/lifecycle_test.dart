...
void main() {
  testWidgetsWithLeakTracking(
    ...
    binding
        .readTestInitialLifecycleStateFromNativeWindow(); // Re-attempt the initialization.

    // The lifecycleState should now be the state we passed above,
    // even though no lifecycle event was fired from the platform.
    expect(
        binding.lifecycleState.toString(), equals('AppLifecycleState.paused'));
  });
  testWidgetsWithLeakTracking(
    ...
    for (final AppLifecycleState state in AppLifecycleState.values) {
      ...
      expect(ServicesBinding.instance.lifecycleState.toString(),
          equals(state.toString()));
    }
  });
  test(
      'AppLifecycleState values are in the right order for the state machine to be correct',
      () {
    expect(
