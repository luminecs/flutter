...
void main() {
  ...
  testWidgetsWithLeakTracking(
      'KeyboardManager synthesizes modifier keys in rawKeyData mode',
      (WidgetTester tester) async {
  ...
  testWidgetsWithLeakTracking(
    ...
    expect(
        await ServicesBinding.instance.keyEventManager
            .handleRawKeyMessage(<String, dynamic>{
          'type': 'keydown',
          'keymap': 'macos',
          'keyCode': 0x00000039,
          'characters': '',
  ...
  testWidgetsWithLeakTracking(
      'debugPrintKeyboardEvents causes logging of key events',
      (WidgetTester tester) async {
    ...
    expect(messagesStr,
        contains('KEYBOARD: Pressed state before processing the event:'));
    expect(messagesStr,
        contains('KEYBOARD: Pressed state after processing the event:'));
  });
}
