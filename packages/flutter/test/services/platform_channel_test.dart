...
void main() {
  ...
  group('BasicMessageChannel', () {
    ...
    const BasicMessageChannel<String?> channel =
        BasicMessageChannel<String?>('ch', string);
  ...
  group('MethodChannel', () {
    ...
    const OptionalMethodChannel optionalMethodChannel =
        OptionalMethodChannel('ch8', jsonMethod);
    tearDown(() {
        ...
        throwsA(
          isA<PlatformException>()
              ...
              .having((PlatformException e) => e.details, 'details',
                  equals(<String, dynamic>{'a': 42, 'b': 3.14})),
        ),
      );
    });
    ...
    test('can invoke unimplemented method (optional)', () async {
      ...
      final String? result =
          await optionalMethodChannel.invokeMethod<String>('sayHello', 'hello');
      expect(result, isNull);
    });

    test('can handle method call with no registered plugin (setting before)',
        () async {
      channel.setMethodCallHandler(null);
      final ByteData call =
      ...
      ByteData? envelope;
      await TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .handlePlatformMessage('ch7', call, (ByteData? result) {
        envelope = result;
      });
    ...
    test('can handle method call with no registered plugin (setting after)',
        () async {
      final ByteData call =
          jsonMethod.encodeMethodCall(const MethodCall('sayHello', 'hello'));
      ByteData? envelope;
    ...
    test('can check the mock handler', () async {
      ...
      expect(
          TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
              .checkMockMessageHandler(channel.name, handler),
          false);
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
    ...
    test('can receive error event', () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        ...
        MockStreamHandler.inline(
          onListen: (Object? arguments, MockStreamHandlerEventSink events) {
            events.error(
                code: '404', message: 'Not Found.', details: arguments);
          },
        ),
      );
