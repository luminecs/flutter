...
Future<void> verifyMarkedNeedsLayoutDuringTransientCallbacksPhase(
    WidgetTester tester, RenderObject renderObject) async {
...
void main() {
  testWidgets('RenderParagraph relayout upon system fonts changes',
    ...
    final RenderObject renderObject =
        tester.renderObject(find.text('text widget'));
    await verifyMarkedNeedsLayoutDuringTransientCallbacksPhase(
        tester, renderObject);
  });
  ...
  testWidgets('RenderEditable relayout upon system fonts changes',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
    ...
    await verifyMarkedNeedsLayoutDuringTransientCallbacksPhase(
        tester, state.renderEditable);
  });

  testWidgets('Banner repaint upon system fonts changes',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      const Banner(
        message: 'message',
  ...
  testWidgets(
    ...
    final Map<int, double> cache =
        state.estimatedColumnWidths as Map<int, double>;
  ...
  testWidgets(
    ...
    // ignore: avoid_dynamic_calls
    expect(state.numberLabelBaseline,
        lessThan(18.400070190429688 + precisionErrorTolerance));
  ...
  testWidgets('RangeSlider relayout upon system fonts changes',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: Material(
    ...
    final RenderObject renderObject = tester.renderObject(
        find.byWidgetPredicate((Widget widget) =>
            widget.runtimeType.toString() == '_RangeSliderRenderObjectWidget'));
    await verifyMarkedNeedsLayoutDuringTransientCallbacksPhase(
        tester, renderObject);
  });
