...
void main() {
  ...
  group('ResizeImage', () {
    group('resizing', () {
      ...
      group('with policy=fit and allowResize=false', () {
        test('constrains square image to bounded portrait rect', () async {
          final MemoryImage rawImage =
              MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 25, height: 50, policy: ResizeImagePolicy.fit);
        ...
        test('constrains square image to bounded landscape rect', () async {
          ...
          await _expectImageSize(rawImage, const Size(50, 50));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 50, height: 25, policy: ResizeImagePolicy.fit);
          await _expectImageSize(resizedImage, const Size(25, 25));
        });
        ...
        test('constrains square image to bounded square', () async {
          ...
          await _expectImageSize(rawImage, const Size(50, 50));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 25, height: 25, policy: ResizeImagePolicy.fit);
          await _expectImageSize(resizedImage, const Size(25, 25));
        });
        ...
        test('constrains square image to bounded width', () async {
          ...
          final ResizeImage resizedImage =
              ResizeImage(rawImage, width: 25, policy: ResizeImagePolicy.fit);
          await _expectImageSize(resizedImage, const Size(25, 25));
        });
        ...
        test('constrains square image to bounded height', () async {
          ...
          await _expectImageSize(rawImage, const Size(50, 50));
          final ResizeImage resizedImage =
              ResizeImage(rawImage, height: 25, policy: ResizeImagePolicy.fit);
          await _expectImageSize(resizedImage, const Size(25, 25));
        });

        test('constrains portrait image to bounded portrait rect', () async {
          final MemoryImage rawImage =
              MemoryImage(Uint8List.fromList(kBluePortraitPng));
          await _expectImageSize(rawImage, const Size(50, 100));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 25, height: 60, policy: ResizeImagePolicy.fit);
        ...
        test('constrains portrait image to bounded landscape rect', () async {
          ...
          await _expectImageSize(rawImage, const Size(50, 100));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 60, height: 25, policy: ResizeImagePolicy.fit);
        ...
        test('constrains landscape image to bounded portrait rect', () async {
          final MemoryImage rawImage =
              MemoryImage(Uint8List.fromList(kBlueLandscapePng));
        ...
        test('leaves image as-is if constraints are bigger than image',
            () async {
          final MemoryImage rawImage =
              MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 120, height: 100, policy: ResizeImagePolicy.fit);
      ...
      group('with policy=fit and allowResize=true', () {
        ...
        test('constrains square image to bounded width', () async {
          ...
          await _expectImageSize(rawImage, const Size(50, 50));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 100, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 100));
        });
        ...
        test('constrains square image to bounded height', () async {
          ...
          await _expectImageSize(rawImage, const Size(50, 50));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              height: 100, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 100));
        });
        ...
        test('constrains portrait image to bounded height', () async {
          ...
          await _expectImageSize(rawImage, const Size(50, 100));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              height: 200, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 200));
        });
        ...
        test('constrains landscape image to bounded width', () async {
          ...
          await _expectImageSize(rawImage, const Size(100, 50));
          final ResizeImage resizedImage = ResizeImage(rawImage,
              width: 200, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(200, 100));
        });
    ...
    test('does not resize when no size is passed', () async {
      ...
      final ImageProvider<Object> resizedImage =
          ResizeImage.resizeIfNeeded(null, null, imageProvider);
    ...
    test('forms closure', () async {
      final Uint8List bytes = Uint8List.fromList(kTransparentImage);
      final MemoryImage memoryImage = MemoryImage(bytes);
      final ResizeImage resizeImage = ResizeImage(memoryImage,
          width: 123, height: 321, allowUpscaling: true);
...
Future<void> _expectImageSize(
    ImageProvider<Object> imageProvider, Size size) async {
