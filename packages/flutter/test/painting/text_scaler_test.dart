...
void main() {
  group('Linear TextScaler', () {
    test('equality', () {
      ...
      final TextScaler c =
          TextScaler.linear(3.0); // ignore: prefer_const_constructors
    ...
    test('clamping', () {
      expect(TextScaler.noScaling.clamp(minScaleFactor: 3.0),
          const TextScaler.linear(3.0));
      expect(const TextScaler.linear(5.0).clamp(maxScaleFactor: 3.0),
          const TextScaler.linear(3.0));
      expect(const TextScaler.linear(5.0).clamp(maxScaleFactor: 3.0),
          const TextScaler.linear(3.0));
      expect(
          const TextScaler.linear(5.0)
              .clamp(minScaleFactor: 3.0, maxScaleFactor: 3.0),
          const TextScaler.linear(3.0));
      // Asserts when min > max.
      expect(
        () => TextScaler.noScaling
            .clamp(minScaleFactor: 5.0, maxScaleFactor: 4.0),
