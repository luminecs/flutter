...
typedef SimpleAnnotationFinder = Iterable<HitTestTarget> Function(
    Offset offset);

void main() {
  final TestMouseTrackerFlutterBinding binding =
      TestMouseTrackerFlutterBinding();
  ...
  setUp(() {
    ...
    binding.defaultBinaryMessenger.setMockMethodCallHandler(
        SystemChannels.mouseCursor, (MethodCall call) async {
  ...
  test('Should work on platforms that does not support mouse cursor', () async {
    const TestAnnotationTarget annotation =
        TestAnnotationTarget(cursor: SystemMouseCursors.grabbing);
  ...
  test('pointer is added and removed out of any annotations', () {
    ...
    setUpMouseTracker(
      annotationFinder: (Offset position) =>
          <TestAnnotationTarget>[if (annotation != null) annotation],
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
          device: 0, kind: SystemMouseCursors.basic.kind),
    ]);
    logCursors.clear();
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
          device: 0, kind: SystemMouseCursors.grabbing.kind),
    ]);
    logCursors.clear();
  ...
  test('The first annotation with non-deferring cursor is used', () {
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
          device: 0, kind: SystemMouseCursors.click.kind),
    ]);
    logCursors.clear();
  ...
  test('Pointing devices display cursors separately', () {
    ...
    setUpMouseTracker(
      annotationFinder: (Offset position) sync* {
        if (position.dx > 200) {
          yield const TestAnnotationTarget(
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
          device: 1, kind: SystemMouseCursors.basic.kind),
      _CursorUpdateDetails.activateSystemCursor(
          device: 2, kind: SystemMouseCursors.basic.kind),
    ]);
    logCursors.clear();
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
          device: 1, kind: SystemMouseCursors.click.kind),
    ]);
    logCursors.clear();
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
          device: 2, kind: SystemMouseCursors.click.kind),
    ]);
    logCursors.clear();
    ...
    expect(logCursors, <_CursorUpdateDetails>[
      _CursorUpdateDetails.activateSystemCursor(
          device: 2, kind: SystemMouseCursors.forbidden.kind),
    ]);
    logCursors.clear();
  });
}
...
class _CursorUpdateDetails extends MethodCall {
  const _CursorUpdateDetails(
      super.method, Map<String, dynamic> super.arguments);
  ...
  }) : this('activateSystemCursor',
            <String, dynamic>{'device': device, 'kind': kind});
  @override
    ...
    return other is _CursorUpdateDetails &&
        ...
        other.arguments.entries.every(
          (MapEntry<String, dynamic> entry) =>
              arguments.containsKey(entry.key) &&
