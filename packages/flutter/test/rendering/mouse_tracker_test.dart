...
typedef SimpleAnnotationFinder = Iterable<TestAnnotationEntry> Function(
    Offset offset);
...
void main() {
  ...
  test(
      'should correctly handle when the annotation appears or disappears on the pointer',
      () {
    ...
    setUpMouseAnnotationFinder((Offset position) sync* {
      if (isInHitRegion) {
        yield TestAnnotationEntry(
            annotation, Matrix4.translationValues(10, 20, 0));
      }
    });
    ...
    expect(
        ...
        _equalToEventsOnCriticalFields(<BaseEventMatcher>[
          EventMatcher<PointerEnterEvent>(
              const PointerEnterEvent(position: Offset(0, 100))
                  .transformed(translate10by20)),
        ]));
    events.clear();
  ...
  test(
      'should correctly handle when the annotation moves in or out of the pointer',
      () {
    late bool isInHitRegion;
    final List<Object> events = <PointerEvent>[];
    final TestAnnotationTarget annotation = TestAnnotationTarget(
...
class BaseEventMatcher extends Matcher {
  ...
  bool matches(dynamic untypedItem, Map<dynamic, dynamic> matchState) {
    ...
    if (!(_matchesField(
            matchState, 'kind', actual.kind, PointerDeviceKind.mouse) &&
        _matchesField(
            matchState, 'position', actual.position, expected.position) &&
        _matchesField(matchState, 'device', actual.device, expected.device) &&
        _matchesField(matchState, 'localPosition', actual.localPosition,
  ...
  Description describeMismatch(
    ...
    return mismatchDescription
        ...
        .add(
            " at field `${matchState['field']}`, which doesn't match the expected ")
