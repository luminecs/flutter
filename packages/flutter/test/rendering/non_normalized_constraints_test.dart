...
void main() {
  testWidgets('Stack parsing in non-normalized constraints error',
      (WidgetTester tester) async {
    await tester.pumpWidget(
        const Foo(child: Placeholder()), Duration.zero, EnginePhase.layout);
    final Object? exception = tester.takeException();
    final String text = exception.toString();
    expect(
        text, contains('BoxConstraints has non-normalized width constraints.'));
    expect(
        text,
        contains(
    ...
    expect(text, contains('non_normalized_constraints_test.dart:17:12'));
      ...
      skip:
          kIsWeb); // [intended] stack traces on web are insufficiently predictable
}
