...
import 'package:flutter_api_samples/cupertino/nav_bar/cupertino_sliver_nav_bar.0.dart'
    as example;
...
void main() {
  testWidgets(
    ...
    expect(tester.getBottomLeft(find.text('Contacts').first).dy,
        36.0 + 8.0); // Static part + _kNavBarBottomPadding.
  });
  ...
  testWidgets('Middle widget is visible in both collapsed and expanded states',
    ...
    expect(tester.getBottomLeft(find.text('Contacts Group').first).dy, 30.5);
    expect(tester.getBottomLeft(find.text('Family').first).dy,
        36.0 + 8.0); // Static part + _kNavBarBottomPadding.
  });
