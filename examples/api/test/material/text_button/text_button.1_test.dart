...
void main() {
  testWidgets('SelectableButton', (WidgetTester tester) async {
    ...
    expect(buttonMaterial().textStyle!.color,
        const ColorScheme.light().primary); // default button foreground color
    expect(buttonMaterial().color,
        Colors.transparent); // default button background color
    ...
    expect(buttonWidget().selected, false);
    expect(
        buttonMaterial().textStyle!.color, const ColorScheme.light().primary);
