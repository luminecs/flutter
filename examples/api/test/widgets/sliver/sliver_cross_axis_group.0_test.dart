...
void main() {
  testWidgets('SliverCrossAxisGroup example', (WidgetTester tester) async {
    ...
    final RenderSliverCrossAxisGroup renderSliverGroup =
    ...
    expect(
        secondList.constraints.crossAxisExtent, equals(expectedSecondExtent));
    expect(thirdList.constraints.crossAxisExtent, equals(expectedThirdExtent));

    // Also check that the paint offsets are correct.
    final RenderSliverConstrainedCrossAxis renderConstrained =
        tester.renderObject<RenderSliverConstrainedCrossAxis>(
            find.byType(SliverConstrainedCrossAxis));

    expect((firstList.parentData! as SliverPhysicalParentData).paintOffset.dx,
        equals(0));
    expect(
    ...
    expect((thirdList.parentData! as SliverPhysicalParentData).paintOffset.dx,
        equals(expectedFirstExtent + expectedSecondExtent));
  });
}
